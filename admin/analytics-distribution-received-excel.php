...
xlsWriteLabel(0,0,"Distribuciones a la UN: ".$unit." del: ".$from1." al: ".$to1);
...
if($unit != ""){
	$sql3 = " and distribution.unit = '$unit'";
}

$sql = $sql1.$sql2.$sql3;

$query = "select payments.id, payments.currency, times.today, payments.description, payments.btype, payments.provider, payments.collaborator, payments.route, distribution.total from payments inner join distribution on payments.id = distribution.payment inner join times on payments.id = times.payment where payments.distributable = '1'".$sql." group by distribution.id";   
...
while($row=mysqli_fetch_array($result)){
	...
	$rowunit = mysqli_fetch_array(mysqli_query($con, "select units.name, units.company from units where code = '$row[7]'"));
	$theunit = $row[7]." | ".$rowunit[0];  
