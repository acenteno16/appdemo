...
if($sOption == 1){
	...
	for($i=0;$i<sizeof($batch);$i++){
		$queryInsert = "insert into bankingDebtRecords (bankingDebt, transaction, batch, document, url) values ('$id', '$idTransaction', '$batch[$i]', '$document[$i]', '$url[$i]')";
		$resultInsert = mysqli_query($con, $queryInsert);
	}
	$newStatus = '2';
}
elseif(($sOption == 2) or ($sOption == 3)){
	...
	if($sOption == 2){
		...
		#Balance del Desembolso
		$queryBalance = "select interest, amount, balance from bankingDebtBalance where bankingDebt = '$id' and transaction = '$idTransaction' order by id desc limit 1";
		$resultBalance = mysqli_query($con, $queryBalance);
		$rowBalance = mysqli_fetch_array($resultBalance);
		
		$newInterest = -1*abs($rowBalance['interest']);
		$newAmount = -1*abs($rowBalance['amount']);
		$newBalance = $rowBalance['balance']+$rowBalance['amount'];
		
		#Save new balance
		$queryBalanceSave = "insert into bankingDebtBalance (today, totime, bankingDebt, userid, type, transaction, interest, amount, balance) values ('$today', '$totime', '$id', '$_SESSION[userid]', '1', '$idTransaction', '$newInterest', '$newAmount', '$newBalance')";
	...
	exit('<script>alert("Debe de seleccionar una opcioón.");history.go(-1);</script>');
}
