...
<div class="row"></div>
<h3  class="form-section">Historial de transacciones</h3> 

<?

$queryTransactionHistory = "select bankingDebtBalance.*, bankingDebtTransactions.type as ttype, bankingDebtTransactions.amortization, bankingDebtTransactions.bankingmovement, bankingDebtTransactions.reference, bankingDebtTransactions.void, bankingDebtTransactions.reason, bankingDebtTransactions.bill, bankingDebtTransactions.promissory,bankingDebtTransactions.dateBank from bankingDebtBalance inner join bankingDebtTransactions on bankingDebtBalance.transaction = bankingDebtTransactions.id where bankingDebtBalance.bankingDebt = '$row[id]'";
$resultTransactionHistory = mysqli_query( $con, $queryTransactionHistory );
$numTransactionHistory  = mysqli_num_rows($resultTransactionHistory);
...
<? while($rowTransactionHistory = mysqli_fetch_array($resultTransactionHistory)){ 
	...
	<tr <? if($rowTransactionHistory['void'] == 1){ echo 'class="danger"'; } ?>> 
		...
		<td><a href="javascript:showMore(<? echo $rowTransactionHistory['id']; ?>);" class="btn btn-xs default btn-editable"><i class="fa fa-angle-down"></i> Ver Transacción</a> <? /*if($_SESSION['email'] == 'jairovargasg@gmail.com'){*/ ?><a class="btn btn-xs red btn-editable" href="javascript:bankingDebtVoid(<? echo $rowTransactionHistory['id']; ?>);"><i class="fa fa-trash-o"></i> Anular transaccion</a><? /*}*/ ?></td>
    ...
    <div class="form-group">
      ...
      <div class="input-group" id="amortizationText">
        <input type="text" id="amortizationUrlHistory" name="amortizationUrlHistory" class="form-control" value="<? echo activeDomain($rowTransactionHistory['amortization']); ?>" readonly>
        <span class="input-group-addon"> <a href="<? echo activeDomain($rowTransactionHistory['amortization']); ?>" target="_blank"><i class="fa fa-search"></i></a> </span> </div>
    </div>
  </div>
  	<? #Movimiento bancario ?>
  	<div class="col-md-3 ">
    <div class="form-group">
      ...
      <div class="input-group" id="amortizationText">
        <input type="text" id="amortizationUrlHistory" name="amortizationUrlHistory" class="form-control" value="<? echo activeDomain($rowTransactionHistory['bankingmovement']); ?>" readonly>
        <span class="input-group-addon"> <a href="<? echo activeDomain($rowTransactionHistory['bankingmovement']); ?>" target="_blank"><i class="fa fa-search"></i></a> </span> </div>
    </div>
  </div>
  	<? #Referencia ?>
  	<div class="col-md-3 ">
    ...
    <div class="form-group">
      ...
      <input type="text" id="dateBank" name="dateBank" class="form-control" value="<? if($rowTransactionHistory['dateBank'] != '0000-00-00') echo date("d-m-Y", strtotime($rowTransactionHistory['dateBank'])); else echo '-'; ?>" readonly>
    </div>
  </div>
  ...
  <? #Pagaré 
   ...
   <div class="col-md-3">
      <div class="form-group">
        <label>Pagaré:</label>
        <div class="input-group">
          <input type="text" id="bdPromissory" name="bdPromissory" class="form-control" value="<? echo activeDomain($rowTransactionHistory['promissory']); ?>" readonly>
          <span class="input-group-addon"> <a href="<? echo activeDomain($rowTransactionHistory['promissory']); ?>" target="_blank"><i class="fa fa-search"></i></a></span> 
		</div>
        </div>
    </div>
    <? } ?>    
    ...
    <div class="row"></div> 
            ...
            if($numBills > 0){
            ?>
            <h3>Factura(s) y/o Documento(s):</h3>
            <div class="row">
            <? 
...
<script>
    ...
    function bankingDebtVoid(transaction){
        
        if(confirm('Esta seguro de anular este desembolso?') == true){
            var reason = window.prompt('Ingrese el motivo de anulacion.');
            window.location = 'bankingDebtVoid.php?id=<? echo $row['id']; ?>&transaction='+transaction+'&reason='+reason;
        }
