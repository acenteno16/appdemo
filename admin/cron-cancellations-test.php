...
$query_main = "select id, btype, provider, collaborator, intern, client from payments where cnotification = '1'";
$query_main = "select id, btype, provider, collaborator, intern, client from payments where ((id = '52556') or (id = '63084') or (id = '67606')) order by id desc limit 10 ";
...
foreach ($providers as $beneficiary => $payments) {
	...
	while($row_payments = mysqli_fetch_array($result_payments)){
		...
		if($row_payments['ret2a'] > 0){
			...
			$row_retention = mysqli_fetch_array($result_retention);
	...
	// Add a recipient 
	//$mail->addAddress('jairovargasg@gmail.com', 'Jairo Vargas');               // Name is optional
	...
	//$mail->addAddress($providers[$p]);     // Add a recipient
