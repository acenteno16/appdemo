...
$query = "select * from development where status = '0' order by id desc"; 
$result = mysqli_query($con, $query);
$numdev = mysqli_num_rows($result);
$totpagina = ceil($numdev / $tampagina);       

$query1 = "select * from development where status = '0' order by id desc limit ".$inicio.",".$tampagina;
...
if($numdev > 0){  ?>
                                ...
                                <tr role="row" class="odd <? 
									if($row['status'] == 4) echo "success"; 
									if($row['status'] == 1) echo "danger";
									if($row['status'] == 3) echo "warning";
									if($row['status'] == 2) echo "active";
													  ?>">
                                <td class="sorting_1"><?php echo $row['id']; ?></td>
                                <td><?php echo date('d-m-Y', strtotime($row['today']));
									//." @".$row['totime']; ?></td>
...
?> </td>
                          ...
                          <td>
                            ...
                            <a href="development-view.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Ver</a>   <a href="javascript:deleteDevelopment(<?php echo $row['id']; ?>)" class="btn btn-xs red btn-editable"><i class="fa fa-trash-o"></i> Eliminar</a>          
                 ...
                 <li>
                  ...
                  <?php } ?>
						...
						<p>
							...
							NOTA: No hay ningúbn desarrollo pendiente.
...
$query1 = "select * from development where status > '0' order by status desc, id desc limit ".$inicio.",".$tampagina;
$result1 = mysqli_query($con, $query1); 
if($pagina < $totpagina) $next = $pagina+1;
if($pagina > 1) $previous = $pagina-1;							
