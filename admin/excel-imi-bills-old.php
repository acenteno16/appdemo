...
for($r0=0;$r0<$idsize;$r0++){
	...
	$queryret = "select payment, number, serial, void from hallsretention where id = '$id[$r0]'";
  	$resultret = mysqli_query($con, $queryret);
  	$rowret = mysqli_fetch_array($resultret);
	
	if(!in_array($rowret['payment'],$paymentArr)){
	
	$paymentArr[] = $rowret['payment'];
	...
	$billnumbers = "";
	$querybills = "select number, billdate, ret1a, stotal, stotal2, exempt, tc, dtype, ret1 from bills where payment = '$row[id]' and ret1a > '0'"; 
