			...
			<div class="row">
				...
				<div class="col-md-12">
					...
					<li>
							...
							<a href="file-review-detail.php">Solicitudes pendientes de revisión</a>
						<i class="fa fa-angle-right"></i>

						</li>
                            

						<li><a href="#">Revisión de solicitudes</a>
...
$gvisor = 1;
$checker = 1;
$queryreview = "select * from filereview where payment = '$row[id]'";
...
<? 
$pCancel = 0;
$pCancelStr = 'Para rechazar/regresar esta solicitud, usted debera de llenar la información de adicional. Si desea rechazar o regresar esta solicitud presione aceptar.';
if($row['status'] == 14){
    $pCancel = 1;
    $pCancelStr = 'Para revisar esta solicitud con err, usted debera de llenar la información de adicional. Si desea rechazar o regresar esta solicitud presione aceptar.';
}                                                                                       
?>
  <input type="hidden" name="pCancel" id="pCancel" value="<? echo $pcancel; ?>">                                          
<div class="col-md-12 " style="display:none;" id="cdiv">


													<div class="form-group">

									 				  <label class="control-label">Opción de <? if($pCancel == 1) echo 'revisar con err/'; ?>rechazar/regresar:</label>

													  <select name="foption" class="form-control" id="foption" onChange="javascript:divShow(this.value);">
<option value="0">Seleccionar</option>
<? if($pCancel == 0){ ?>                                                          
<option value="1">Regresar a Provisión</option>
<option value="2">Rechazar Solicitud</option>
<? }else{ ?>                                                         
<option value="3">Revisar con Err</option> 
<? } ?>                                                         
...
<button type="button" class="btn red" onClick="denyPayment();"><i class="fa fa-times"></i> <? if($pCancel == 1){ echo 'Revisar con Err'; }else{ ?>Rechazar/Regresar Pago<? } ?></button> &nbsp; &nbsp; &nbsp; &nbsp;<input type="submit" value="Revisar" id="rpay"> <input type="button" value="Cancelar" id="rpay2" style="display:none;" onClick="cancelAction();"> 
</div>
</form>
</div>
...
<script>
			...
			function cancelAction(){
				...
				var divrpay = document.getElementById('rpay2').style.display;
				 
				
				if(divdeny == 'block'){
					document.getElementById('rpay').style.display = "block";
					document.getElementById('rpay2').style.display = "none";
			...
			window.location="payment-review-detail-check-deny.php?id=<?php echo $_GET['id']; ?>&foption="+foption+"&reason="+reason+"&reason2="+reason2;    
		}else{
			document.getElementById('cdiv').style.display = "none";
		}
				}
				...
				if(confirm('<? echo $pCancelStr; ?>') == true){
				document.getElementById('rpay').style.display = "none";
				document.getElementById('rpay2').style.display = "block";
