...
if(!$stage){ $sql = " and sent = '3'"; } 
elseif($stage == 1){ $sql = " and sent >= '2' and sent < '5'"; } 
elseif($stage == 2){ $sql = " and sent_complete = 1"; }
elseif($stage == 3){ $sql = " and sent = '4' and sent_complete = '0'"; }
elseif($stage == 4){ $sql = " and sent = '4' and sent_complete = '0'"; }
elseif($stage == 5){ $sql = " and sent_complete = '0' and sent-approve = '1'"; }
							
$query = "select payments.* from payments left join providers on payments.provider = providers.code where payments.id > 0".$sql." order by providers.flag desc group by payments.id";
$result = mysqli_query($con, $query);
$numdev = mysqli_num_rows($result);
$totpagina = ceil($numdev / $tampagina);
			
$query1 = "select payments.* from payments left join providers on payments.provider = providers.code where payments.id > 0".$sql." order by providers.flag desc"; 
...
?>
		...
		$rid = $rowpackages['id'];
								...
								?>"> 
                                <td class="sorting_1"> <?php echo $rid; ?></td>
                               ...
                               <?php $rowprovider = mysqli_fetch_array(mysqli_query($con, "select * from providers where id = '$row[provider]'"));
                                ...
                                ?></td>
                                <td>
                                <?php echo $rowcurrency['pre'].' '.$rowcurrency['symbol'].str_replace('.00','',number_format($row['payment'], 2));
								/*.' '.$rowcurrency['name'];*/ ?></td>
                                <td>
                                <?php //VENCIMIENTO ?>
...
?>
                 ...
                 <li>
										<a href="file-review-detail.php?stage=<? echo $_GET['stage']; ?>&page=<?php echo $previous; ?>">
  ...
  for ($i=1;$i<=$totpagina;$i++){ 
        if ($pagina == $i){
		  ...
		  echo '<li><a href="file-review-detail.php?stage='.$_GET['stage'].'&page='.$i .'">'.$i .'</a></li>'; 
		}
    } } ?>
             <?php if($next != ""){ ?>
                 ...
                 <li>
										<a href="file-review-detail.php?stage=<? echo $_GET['stage']; ?>&page=<?php echo $next; ?>);">
										<i class="fa fa-angle-right"></i>
										</a>
									</li>
                  <?php } ?>
