...
function reloadNumbers(){  	
	...
	if (currency[i].checked) {
    	var billcurrency = currency[i].value;re
	}
	} 
	if(!billcurrency){
	...
	for (var i = 0; i < document.getElementsByName('stotal[]').length; i++) {
   		...
   		var p2global = document.getElementsByName('billrettype[]')[i].value;
		p2global = p2global.split(",");
		var p2 = p2global[1];
...
if((currency2pay == 2) && (billcurrency == 2)){
		...
		document.getElementById('payment').value = commas(totalpay.toFixed(2))+' Dólares'; 
