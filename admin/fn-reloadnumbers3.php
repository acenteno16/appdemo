...
for (var i = 0; i < document.getElementsByName('stotal[]').length; i++) {
  ...
  var billdate = document.getElementsByName('billdate[]')[i].value; 
   ...
   //bill retention
   //si las facturas son en dolares
   if(billcurrency == 2){
   ...
   //Sub-total exento NIO
   var stotalexemptnio = 0;
   var stotalexemptnio = parseFloat(stotalbillnio); 
   if(parseFloat(exempt)){
	   stotalexemptnio -= parseFloat(exempt)*parseFloat(tc);
   }
   //Okay
   ...
   if(parseFloat(totalbill) > 0){
	   ...
	   var gtotalbillnio = parseFloat(gtotalbillnio)+parseFloat(totalbillnio);
   }
   */  
   ...
   if(stotalbillnio >= 1001){ 
		...
		//Si hay algun valor en el campo retencion alcaldia
		if(p1 != ""){
  ...
  */
   ...
   if(parseFloat(totalbill) > 0){
	   ...
	   gtotalbillnio += parseFloat(gtotalbill)*parseFloat(tc)
   }
   ...
   if(parseFloat(inturammount) > 0){
	...
	//Proveedor en Dolares // Pago en Dolares //Cancelacion en Dolares
