...
function reloadNumbers(){  	
	...
	loadcurrency2pay(dspayment,provider,nochange);
	
	<?php //read globals
	
	//we need connection to database
	
	include('../connection.php');
	
	$queryglobals = "select * from config";
...
for (var i = 0; i < document.getElementsByName('stotal[]').length; i++) {
   ...
   var billdate_d = getDates(billdate);
   if(billdate_d == 1){
   ...
   var billdate2 = document.getElementsByName('billdate2[]')[i].value;
   var billdate2_d = getDates(billdate2);
   if(billdate2_d == 1){
   ...
   ///////////////////////
	...
	if((document.getElementById('retainer2').checked == true) || (document.getElementById('retainer3').checked == true)){
		document.getElementById('acpdiv').style.display = "block"; 
	}else{
		document.getElementById('acpdiv').style.display = "none";
	}
	
	var acptype = document.getElementsByName('acptype');
	for (var a = 0, length = acptype.length; a < length; a++) {
    
	if (acptype[a].checked) { 
    	var cacptype = acptype[a].value; 
	}
	} 
   ...
   if(dspayment == '1' && (stotalbillnio >= 1000 || theconceptr == '99')){   
...
function getDates(tdate) {
    ...
    $.post("reload-compare-dates.php", { thedate: tdate }, function(data) {
...
function loadcurrency2pay(dspayment2pay,provider,nochange){
...
$.ajaxSetup({async:false});
	...
	if(nochange == 0){	
		...
		$.post("reload-currency2pay.php", { variable: provider }, function(data){
			...
			<?
	//Solo para mi
	if($_SESSION['email'] == 'mtorres@casapellas.com.ni'){
	?>
	//alert(data);  
