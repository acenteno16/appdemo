...
function notifyCancellation($payment,$forwarding,$noRequire,$forcedEmail,$con){ 
	...
	if($rowpayment['btype'] == 1){
		...
		//Colaborador
		$queryprovider = "select * from workers where id = '$rowpayment[collaborator]'";
		$resultprovider = mysqli_query($con, $queryprovider);
		$rowprovider = mysqli_fetch_array($resultprovider);
	...
	$companyLogo = "/var/www/html/assets/admin/companies/$company-email.png";
	if(!file_exists($companyLogo)){
		$companyLogo = '/var/www/html/assets/admin/companies/grupo-casa-pellas.png';
	}
	...
	if($forwarding == 0){
		...
		$queryErr = "insert into mailerLog (payment, type, status, today, sent, message, pagename) values ('$payment', '3', '$x', '$now', '$emailSent', '$emailStatusMessage', '$pagename')";
		$resultErr = mysqli_query($con, $queryErr);
