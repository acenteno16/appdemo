		...
		<div class="page-content">
			...
			<div class="row">
				...
				<div class="col-md-12">
					...
					Transferencias Bancarias <? //<small>Solicitudes de pago</small> ?>
					...
					<div class="portlet">
						...
						<div class="portlet-title">
							...
							<div class="actions">
								...
								<a href="payment-order-letters-add.php" class="btn default blue-stripe">
...
$query = "select * from letters where userid = '$_SESSION[userid]' order by id desc";
$result = mysqli_query($con, $query);
$numdev = mysqli_num_rows($result);
$totpagina = ceil($numdev / $tampagina);

$query1 = "select * from letters where userid = '$_SESSION[userid]' order by id desc limit ".$inicio.",".$tampagina;  
...
if($numdev > 0){  ?>
								...
								$row_account1 = mysqli_fetch_array(mysqli_query($con, "select companies.name, banks.name, banksaccounts.account2, currency.name, banksaccounts.id from banksaccounts inner join banks on banksaccounts.bank = banks.id inner join currency on banksaccounts.currency = currency.id inner join companies on banksaccounts.company = companies.id where banksaccounts.id = '$row[account1]'"));
								
								$row_account2 = mysqli_fetch_array(mysqli_query($con, "select companies.name, banks.name, banksaccounts.account2, currency.name, banksaccounts.id from banksaccounts inner join banks on banksaccounts.bank = banks.id inner join currency on banksaccounts.currency = currency.id inner join companies on banksaccounts.company = companies.id where banksaccounts.id = '$row[account2]'"));
                                ...
                                <td><? if($row['account1'] > 0){ echo "$row_account1[0] | $row_account1[1] - $row_account1[2] ($row_account1[3])"; } ?></td>
                                <td><? if($row['account2'] > 0){ echo "$row_account2[0] | $row_account2[1] - $row_account2[2] ($row_account2[3])"; } ?></td>
                                <td><? echo $row['amount']; ?></td>
								<td>
                            <?php if($row['status'] == 0){ ?> 
                            <a href="payment	-order-letters.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-edit"></i> Completar</a>
                            <?php }else{ ?>
                            
                            <a href="payment-letters-view.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Ver</a>
                            <? } ?>
