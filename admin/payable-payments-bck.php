...
$query = "select schedule.* from schedule$join where (schedule.status = '3' or schedule.status = '5')".$sql." order by schedule.id asc"; 
$result = mysqli_query($con, $query);  
$num = mysqli_num_rows($result); 
$numdev = mysqli_num_rows($result);
$totpagina = ceil($numdev / $tampagina);
                                
$query1 = "select schedule.* from schedule$join where (schedule.status = '3' or schedule.status = '5')".$sql." order by schedule.id asc limit ".$inicio.",".$tampagina;  
