...
require("session-payer.php"); 
require("functions.php");

$id = isset($_GET['id']) ? sanitizeInput(intval($_GET['id']), $con) : 0;

$querygroup = "select * from schedule where id = ?";
$stmtgroup = $con->prepare($querygroup);
...
if($rowgroup['status'] != 3){
	exit('<script>alert("El grupo debe de estar ingresado a banco."); window.location = "payable-payments.php";</script>');
}
                  ...
                  <div class="col-md-12"><!-- Begin: life time stats -->
                    ...
                    <div class="portlet-body">
                             ...
                             <?php /*<form action="withholding-mayor-tax-request-code.php" method="post" enctype="multipart/form-data" name="form2" id="form2"><div class="table-container">*/ ?>
								...
								if($nummain > 0){ ?>
							<p><strong>Grupo de Cancelación:</strong> <?php echo "#".sanitizedOutput($id); ?><br>
                        	<strong>Web ID:</strong> <?php echo sanitizedOutput($rowgroup['code']); ?><br>
							<? if($rowgroup['bank'] != 0){ ?> Banco: <? 
									echo sanitizedOutput($rowgroup['bank']);
								} 
							?>
							<strong>Elborado por:</strong> <?php echo sanitizedOutput($rowuser['code'])." | ".sanitizedOutput($rowuser['first'])." ".sanitizedOutput($rowuser['last']); ?><br>
	 ...
	 var checkall = document.getElementById('checkall0');
      ...
      for (var i = 0; i < checkboxes.length; i++) {
		   ...
		   activateSelect(i);
                                ...
                                <tr role="row" class="odd <?php if($row['status'] == 14) echo "success"; ?>"> 
									...
									<td class="sorting_1"> 
										...
										<input name="theid[]" type="checkbox" id="theid-<? echo $inc; ?>" value="<?php echo $row['id']; ?>" class="approve0" <? if($rowLetters['parent'] > 0) echo 'disabled'; ?>  onChange="activateSelect(<? echo $inc; ?>);">
                                  ...
                                  <td><a href="payment-order-view.php?id=<?php echo $row['id']; ?>"><span class="sorting_1">
                                   
                                  </span><?php echo $row['id']; ?></a></td>
...
?></td>
							...
							<?php /* <input name="link[]" type="text" class="form-control" id="link[]" value="" placeholder="Link">*/ ?>
                            ...
                            <? 
                                                          ...
                                                          /*if(($row['sent_approve'] == 0)){
                                                              $thisActive = 0;
                                                              if($row['child'] > 0){
                                                                  $queryParent = "select sent_approve from payments where id = '$row[child]'";
                                                                  $resultParent = mysqli_query($con, $queryParent);
                                                                  $rowParent = mysqli_fetch_array($resultParent);
                                                                  
                                                                  if($rowParent['sent_approve'] == 1){
                                                                      $thisActive = 1;
                                                                  }
                                    ...
                                    ?>        
                            <input name="number[]" type="text" class="form-control disableBorder" id="number-<? echo $inc; ?>" value="" placeholder="<? if($thisActive == 1){ echo 'Número de transacción (CKPK)'; }else{ echo "En espera de CC"; } ?>" disabled>
									<? $inc++; ?>
                         <?php //if($rowprovider['international'] != 1){ ?>  
...
<?php //} ?>
                            ...
                            <input name="international[]" type="hidden" id="international[]" value="<?php echo $rowprovider['international']; ?>">
                            ...
                            <?php } ?>
								...
								?>
											    	...
											    	<input name="group" type="hidden" id="group" value="<?php echo sanitizedOutput($id);  ?>">
 <div class="row">                                                   
<div class="col-md-4">  
<label>Archivo:</label>                                                  				<select name="link" class="form-control  select2me" id="link" data-placeholder="Seleccionar..." onChange="javascript:reloadNumbers(),validateBill();"> 
...
?>
												<option value="<?php echo $rowfbox['id'].':::http://getpay.casapellas.com.ni/admin/visor.php?key='.$rowfbox['url']; ?>"><?php echo $rowfbox['id']." | ".$rowfbox['title']; ?></option>
                                                <?php } ?>
...
<div class="col-md-4">
<label>Banco:</label> 
<select name="bank" class="form-control" id="bank" style="margin-top:1px;" <?php if($rowgroup['bank'] > 0) echo "readonly"; ?>>
...
?>
<option value="<?php echo $rowbanks['id']; ?>" <?php if($rowgroup['bank'] == $rowbanks['id']) echo 'selected'; ?>><?php echo $rowbanks['name']; ?></option>
<?php } ?>
...
<div class="col-md-4">
<label>WID:</label> 
 <input name="reference" type="text" class="form-control" id="reference" value="<?php echo $rowgroup['code']; ?>" placeholder="Número de referencia (Banco)">    
 </div>
	 <? /*<div class="row"></div>
	 <div class="col-md-12">
		 <h3 class="form-section">Pendiente E1 </h3>
		 <p>Marque la siguiente casilla si el pago se gurdará como Pendiente de Cancelación en E1</p>
							
							  <input name="pce1" type="checkbox" id="pce1" value="1" <?php if($rowpconfirm['ppe1'] == 1) echo 'checked'; ?>>
							    Pendiente de Cancelaciónen E1

</div> */ ?>
 </div>
  <div class="form-actionsleft" style=" text-align:left;">
 
 <div class="row"></div>
                                            <br>
                                                    
                                         <button type="submit" class="btn blue"><i class="fa fa-check"></i> Cancelar pago</button><br>
                                                    <p style="margin-top:10px;"><strong>Total Grupo:</strong> <?php switch($row['currency']){
								  case 1:
								  $pre = "NIO C$";
								  $currency = "Cordobas";
								  break;
