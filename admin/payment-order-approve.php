...
if(isset($_GET['message'])){
	...
	echo "<script>alert('$message'); window.location = 'payment-order-approve.php'; </script>";
} 
		...
		<div class="page-content">
			...
			<div class="row">
				...
				<div class="col-md-12">
					...
					Pagos <small>Visto bueno de solicitud</small> 
					...
					<div class="portlet">
						...
						<div class="portlet-body">
							...
							<?php 
								...
								$sqlu = "";
								$numu = 0;
								$queryu = "select * from routes where worker = '$_SESSION[userid]' and type = '20'";
								...
								if($numu > 0){
									...
									while($rowu=mysqli_fetch_array($resultu)){
										if($firstu == 1){ //First
											$sqlu = " and (((payments.routeid = '$rowu[unitid]') and (payments.headship2 = '$rowu[headship]'))";
											if($numu == 1){ $sqlu .= ")"; } $firstu++;
										}elseif($firstu == $numu){ //Last
											$sqlu .= " or ((payments.routeid = '$rowu[unitid]') and (payments.headship2 = '$rowu[headship]')))";
											$firstu++;
										}else{ //Middle
											$sqlu .= " or ((payments.routeid = '$rowu[unitid]') and (payments.headship2 = '$rowu[headship]'))";
											$firstu++;
							 	...
							 	$query = "select payments.* from payments inner join workers on payments.userid = workers.code where payments.status = 1 and payments.approved = '0' and payments.arequest = '0' and payments.child='0'".$sqlu." order by payments.expiration asc"; 
								if($_GET['echo'] == 1){
									echo $query;
								}
                                ...
                                <?php //echo '<p style="color:#FFFFFF;">'.$query.'</p>'; ?>
                                
                              <form id="approve1" name="approve1" action="payment-order-approve-view-code.php" method="get" enctype="multipart/form-data">
      ...
      for (var i = 0; i < checkboxes.length; i++) {
			 ...
			 if(checkboxes[i].disabled == true){
			 	checkboxes[i].checked = false; 
                                ...
                                <tr role="row" <?php if($row['expiration'] < date('Y-m-d')) echo 'class="danger"'; ?>><td class="sorting_1"> <input name="id[]" type="checkbox" id="id[]" value="<?php echo $row['id']; ?>" class="approve1" <? if($row['parent'] > 0) echo 'disabled'; ?>></td>
...
?></td><td><?php echo $rowstage['content']; ?> 
										</td><td><a href="payment-order-approve-view.php?id=<?php echo $row['id']; ?>&atype=1" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Ver</a>  <? if($row['parent'] == 0){ ?><a href="payment-order-approve-view-code.php?id[]=<?php echo $row['id']; ?>&approve=1&atype=1" class="btn btn-xs default btn-editable"><i class="fa fa-check"></i> Aprobar</a><? } ?></td></tr>
...
<?php } else { ?>
						...
						<p>
							...
							NOTA: Ninguna solicitud en espera de visto bueno.</p>
...
<?php /*<div class="col-md-12"><!-- Begin: life time stats -->
					...
					<div class="portlet">
						...
						<div class="portlet-body">
							...
							<?php //3
								$sqlu = "";
								$numu = 0;
								$queryu = "select * from routes where worker = '$_SESSION[userid]'";
								$resultu = mysqli_query($con, $queryu);
								$numu = mysqli_num_rows($resultu);
								if($numu > 0){ 
									$firstu = 1;
								...
								//3	
								$query = "select payments.* from payments inner join workers on payments.userid = workers.code where payments.approved = 1 or payments.approved = 2".$sqlu." order by payments.expiration desc"; 
                                ...
                                <td><?php echo $row['id']; ?></td><td><?php echo $rowprovider['code']; ?></td><td><?php if($rowprovider['flag'] == 1) echo '<img src="../images/flag.png" width="13" alt=""/>'; 
								if($row['btype'] == 1){ echo $rowprovider['name'];
								}else{
									echo $rowprovider['first']." ".$rowprovider['last']; }?></td><td><?php $querycurrency = "select * from currency where id = '$row[currency]'"; 
								$resultcurrency = mysqli_query($con, $querycurrency);
								$rowcurrency = mysqli_fetch_array($resultcurrency);
								echo $rowcurrency['pre']." ";
								echo $rowcurrency['symbol'];
...
<?php $day1 = $row['today'];
							...
							$day2 = strtotime('+'.$rowprovider['term'].' day',strtotime($day1));
...
?>
							...
							</td><td><a href="payment-order-view.php?id=<?php echo $row['id']; ?>&atype=3" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Ver</a></td></tr>  
                                <?php } ?>
