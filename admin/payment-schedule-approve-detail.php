			...
			<div class="row">
				...
				<div class="col-md-12">
					...
					<ul class="page-breadcrumb breadcrumb">
                        ...
                        <li>
							<a href="payment-schedule-approve.php">Ingreso a Banco</a>
							<i class="fa fa-angle-right"></i>
						</li>
...
$querybalance = "select * from balance where company = '$_GET[company]' and currency='1' order by id desc limit 1";
                ...
                <div class="col-md-12">
                    ...
                    <div class="portlet">
						...
						<div class="portlet-body">
							...
							$query = "select schedule.* from schedule inner join schedulecontent on schedule.id = schedulecontent.schedule inner join payments on payments.id = schedulecontent.payment inner join units on payments.route = units.code where schedule.status = '1' and schedule.currency = '1' and schedule.vo = '1' and units.company = '$_GET[company]' group by schedule.id order by schedule.id"; 
...
?>
<option value="<?php echo $rowbanks['id']; ?>" <?php if($row['bank'] == $rowbanks['id']) echo 'selected'; ?>><?php echo $rowbanks['name']; ?></option>
<?php } ?> 
 
</select>
</td>
                                  <td><?php echo $rowuser['first']." ".$rowuser['last']; ?></td><td><?php 
								  ...
								  while($rowmain = mysqli_fetch_array($resultmain)){
									  ...
									  if($rowbills1['currency'] != $rowpayment['currency']){
										  ...
										  $querytc = "select * from tc where today = '$scheduletoday'";
								   		  $resulttc = mysqli_query($con, $querytc);
								   		  $rowtc = mysqli_fetch_array($resulttc); 
								   		  $ntc = $rowtc['tc']; 
										  
										  $querybills2 = "select * from bills where payment = '$rowpayment[id]'";
							   ...
							   /*$queryschedulec = "select * from schedulecontent where schedule = '$row[id]'";
							   $resultschedulec = mysqli_query($con, $queryschedulec);
							   while($rowschedulec = mysqli_fetch_array($resultschedulec)){
								   $paymentid = $rowschedulec['payment'];
								   
							   $querybills1 = "select * from bills where payment = '$paymentid' limit 1";
							   $resultbills1 = mysqli_query($con, $querybills1);
							   $rowbills1 = mysqli_fetch_array($resultbills1);
							   
							   if($row['currency'] != $rowbills1['currency']){ 
								   
								   $querycancellation = "select * from times where payment = '$paymentid' and stage = '12.00'";
  								   $resultcancellation = mysqli_query($con, $querycancellation);
  								   $rowcancellation = mysqli_fetch_array($resultcancellation);
  								   $scheduletoday = $rowcancellation['today'];
								   
								   $querybills = "select * from bills where payment = '$paymentid'"; 
							   	   $resultbills = mysqli_query($con, $querybills);
							   	   $numbills = mysqli_num_rows($resultbills);
								   
								   while($rowbills = mysqli_fetch_array($resultbills)){
									  $tbills += $rowbills['ammount']; 
   ...
   ?>
                            ...
                            <td><?php 
								echo $npayments;
								?></td>
                            <td><a href="payment-schedule-approve-group-view.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable" target="_blank"><i class="fa fa-search"></i> Ver</a></td></tr>
                                <?php } ?>
                    ...
                    <div class="portlet">
						...
						<div class="portlet-body">
							...
							$query = "select * from schedule where status = '1' and currency = '2' and vo = '1' order by id desc"; 
								$query = "select schedule.* from schedule inner join schedulecontent on schedule.id = schedulecontent.schedule inner join payments on payments.id = schedulecontent.payment inner join units on payments.route = units.code where schedule.status = '1' and schedule.currency = '2' and schedule.vo = '1' and units.company = '$_GET[company]' group by schedule.id order by schedule.id"; 
								$result = mysqli_query($con, $query);
								$num = mysqli_num_rows($result);
								if($num > 0){ ?> 	
...
<?php } ?>
...
</td><td><?php echo $rowuser['first']." ".$rowuser['last']; ?></td><td><?php $querymain = "select * from schedulecontent where schedule = '$row[id]'"; 
								$resultmain = mysqli_query($con, $querymain);
								$gpayment = 0;
								$npayments = 0;
								while($rowmain = mysqli_fetch_array($resultmain)){
							   ...
							   ?>                                    <input name="tpayment2[]" type="hidden" id="tpayment2[]" value="<?php echo $gpayment; ?>"></td><td><?php  
								echo $npayments;
								?></td><td><a href="payment-schedule-approve-group-view.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable" target="_blank"><i class="fa fa-search"></i> Ver</a></td></tr>
                                <?php } ?>
...
<input type="hidden" id="cbalancefloat2" name="cbalancefloat2" value="0">
                ...
                <div class="portlet">
						...
						<div class="portlet-body">
							...
							<?php $query = "select * from schedule where status = '1' and (currency = '3' or currency = '4') order by schedule asc";
								$query = "select schedule.* from schedule inner join schedulecontent on schedule.id = schedulecontent.schedule inner join payments on payments.id = schedulecontent.payment inner join units on payments.route = units.code where schedule.status = '1' and (schedule.currency = '3' or schedule.currency = '4') and schedule.vo = '1' and units.company = '$_GET[company]' group by schedule.id order by schedule.id"; 
...
<?php } ?>
							   ...
							   ?>
								...
								echo $npayments;
								?></td><td><a href="payment-schedule-group-view.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable" target="_blank"><i class="fa fa-search"></i> Ver</a></td></tr> 
                                <?php } ?>
