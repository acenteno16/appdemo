				...
				<div class="col-md-12">
					...
					<ul class="page-breadcrumb breadcrumb">
						...
						<li>
							...
							<a href="#">Programación de pagos</a></li>
                        <li>
...
$query = "select schedule.* from schedule left join schedulecontent on schedule.id = schedulecontent.schedule left join payments on payments.id = schedulecontent.payment where schedule.id >= '3'".$sql." group by schedule.id order by id desc"; 
$result = mysqli_query($con, $query);
$num = mysqli_num_rows($result); 
$numdev = mysqli_num_rows($result);
$totpagina = ceil($numdev / $tampagina);      

echo $query1 = "select schedule.* from schedule left join schedulecontent on schedule.id = schedulecontent.schedule left join payments on payments.id = schedulecontent.payment where schedule.id >= '3'".$sql." group by schedule.id order by id desc limit ".$inicio.",".$tampagina;
...
<div class="table-scrollable">
      ...
      for (var i = 0; i < checkboxes.length; i++) {
             ...
             if(checkall.checked == true){ 
			   checkboxes[i].checked = true;
                 ...
                 <li>
										<a href="payment-schedule-approve-group.php?page=<?php echo $previous; ?>">
  ...
  for ($i=1;$i<=$totpagina;$i++){ 
        if ($pagina == $i){
		  ...
		  echo '<li><a href="payment-schedule-approve-group.php?page='.$i .'">'.$i .'</a></li>'; 
		}
    } } ?>
             <?php if($next != ""){ ?>
                 ...
                 <li>
										<a href="payment-schedule-approve-group.php?page=<?php echo $next; ?>">
										<i class="fa fa-angle-right"></i>
										</a>
									</li>
                  <?php } ?>
