				...
				<div class="col-md-12">
					...
					<ul class="page-breadcrumb breadcrumb">
                        ...
                        <li>
							...
							<a href="payment-schedule-group.php">Grupos de programación</a>
							<i class="fa fa-angle-right"></i>
...
$query = "select schedule.* from schedule".$join." where schedule.id > '0'".$sql." group by schedule.id order by schedule.id desc";  
$result = mysqli_query($con, $query);
$num = mysqli_num_rows($result); 
$numdev = mysqli_num_rows($result);
$totpagina = ceil($numdev / $tampagina);      

$query1 = "select schedule.* from schedule".$join." where schedule.id > '0'".$sql." group by schedule.id order by id desc limit ".$inicio.",".$tampagina;
...
<td><?php echo date('h:i:s a', strtotime($row['now2'])); ?></td>
                 ...
                 <li>
										<a href="?page=<?php echo $previous.$chain; ?>">
  ...
  for ($i=1;$i<=$totpagina;$i++){ 
        if ($pagina == $i){
		  ...
		  echo '<li><a href="?page='.$i.$chain.'">'.$i.'</a></li>'; 
		}
    } } ?>
             <?php if($next != ""){ ?>
                 ...
                 <li>
										<a href="?page=<?php echo $next.$chain; ?>">
										<i class="fa fa-angle-right"></i>
										</a>
									</li>
                  <?php } ?>
...
<!-- END BODY -->
	...
	function cleanfilert(){
		window.location = "payment-schedule-group-records.php";
	}
	</script>