				...
				<div class="col-md-12">
					...
					<ul class="page-breadcrumb breadcrumb">
                        ...
                        <li>
							...
							<a href="#">Grupos de programación</a>
...
<? 
		...
		if($_GET['showfilter'] == 1){
			include('payment-schedule-group-filter.php'); 
		}else{ ?> 
			<div class="note note-regular"><a href="?showfilter=1">[Mostrar Filtro]</a></div>
<? 		} // End Showfilter 
...
$query = "select schedule.* from schedule left join schedulecontent on schedule.id = schedulecontent.schedule left join payments on payments.id = schedulecontent.payment where schedule.id > '0'".$sql." group by schedule.id order by id desc"; 
$result = mysqli_query($con, $query);
$num = mysqli_num_rows($result); 
$numdev = mysqli_num_rows($result);
$totpagina = ceil($numdev / $tampagina);      

$query1 = "select schedule.* from schedule left join schedulecontent on schedule.id = schedulecontent.schedule left join payments on payments.id = schedulecontent.payment where schedule.id > '0'".$sql." group by schedule.id order by id desc limit ".$inicio.",".$tampagina;
...
$querymy = "select schedule.* from schedule left join schedulecontent on schedule.id = schedulecontent.schedule left join payments on payments.id = schedulecontent.payment where schedule.id > '0' and schedule.userid2 = '$_SESSION[userid]' and vo = '0' group by schedule.id order by id"; 
$resultmy = mysqli_query($con, $querymy);						
$nummy = mysqli_num_rows($resultmy);
						
if(($num > 0) or ($nummy > 0)){ 
?>
<div class="note note-regular">
<p>NOTA: Los grupos de cancelacion, luego de ser programados deben de ser aprobados para que pasen al siguiente etapa "Aprobado de programación". Antes de aprobar un grupo, el procesador de pagos tiene la opcion de modificar el mismo. </p>
<p>Los grupos de cancelación que no esten aprobados se presentan con fondo rojo.</p>
</div>
<? if(($nummy > 0)){ ?>
<div class="portlet">
<div class="portlet-title">
<div class="caption">
Mis Grupos de programación 
</div>
</div>
</div>
...
<div class="table-scrollable">
								...
								while($row=mysqli_fetch_array($resultmy)){
...
<td><?php echo date('h:i:s a', strtotime($row['now2'])); ?></td>
                            ...
                            <td><a href="payment-schedule-group-view.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Ver</a>
                        ...
                        <div class="portlet-title">
						<div class="caption">
							Todos los Grupos de programación
                        </div>
                        </div>
                        </div>
								...
								$rowprovider = mysqli_fetch_array($resultprovider=mysqli_query($con, $queryprovider="select plans.id from plans inner join providers on plans.id = providers.plan inner join payments on providers.id = payments.provider inner join schedulecontent on payments.id = schedulecontent.payment where schedulecontent.schedule = '$row[id]' limit 1"));
								//echo '<br>'.mysqli_num_rows($resultprovider);  
								$plan = ""; 
								
								$queryplan = "select * from plans where id = '$rowprovider[0]'";
								$resultplan = mysqli_query($con, $queryplan);
								$numplan = mysqli_num_rows($resultplan); 
								if($numplan > 0){ 
									while($rowplan=mysqli_fetch_array($resultplan)){
										$plan.= $rowplan['account'].", ";
									}
