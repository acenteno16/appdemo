...
if(($row['status'] == '9.00') or ($row['status'] == '13.02') or ($row['status'] == '13.03')){
	//OKAY
}else{
	?>
    <script>
	alert('La solicitud de pago #'+<?php echo $id; ?>+' se encuentra en otra etapa'); 
...
if($blockschedule == 0){ 
	...
	$queryblock2 = "select * from workers where code = '$row[blockschedule]'";
	...
	echo "
	<script>
	alert('El pago esta siendo programado por $liberatorname'); 
	window.location = 'releasing.php';
	</script>
	";
}else{
	$queryblock = mysqli_query($con, "update payments set blockschedule = '$_SESSION[userid]' where id = '$id'"); 
} 
	...
	<div class="page-content-wrapper">
		...
		<div class="page-content">
           ...
           <div class="portlet"><div class="portlet-title">
                            ...
                            */ ?>
                               <?php if($row['status'] == 9){ ?> <a href="#" class="btn default blue-stripe" onClick="unlockRelease();">
							...
							function unlockRelease(){
								window.location = "payment-schedule-unlock.php?id=<?php echo $_GET['id']; ?>";
							} 
							</script>
...
<form action="payment-schedule-view-code.php" class="horizontal-form" method="post" enctype="multipart/form-data" onsubmit="return validateForm();">                                        									<div class="row">
 <div class="col-md-4">

													<div class="form-group">

											 				<label class="control-label">Opciones de rechazo:</label>

													  <select name="schedule" class="form-control" id="schedule" onChange="javascript:divShow(this.value);">
