...
<br><br>
                    ...
                    <div class="note note-regular">
                    <a href="?filter">Volver a consultar</a> 
                    </div>
                    <div class="portlet">
...
if($param == 0){
	...
	$sql = $sql1.$sql2.$sql3.$sql4.$sql5.$sql6.$sql7.$sql8.$sql9.$sql10.$sql11.$sql12.$sql13.$sql14.$sql15.$sql16.$sqlu; 
...
$query1 = "select payments.id, payments.btype, payments.provider, payments.collaborator, payments.currency, payments.payment, payments.bank, payments.status, payments.reference, payments.cnumber, payments.schedule, payments.approved, payments.today, payments.reason, payments.parent, payments.intern, payments.route, payments.globalpayment, payments.client from payments".$join." where payments.id > '0' and payments.id > 0".$sql." group by times.payment order by payments.id desc limit ".$inicio.",".$tampagina."";
...
if($numdev > 0){  ?>
								...
								elseif($row['btype'] == 4){
									$queryprovider = "select * from clients where code = '$row[client]'";
                                ...
                                <td>
                                <?php if($rowprovider['flag'] == 1) echo '<img src="../images/flag.png" width="13" alt=""/>'; 
								echo $ben_name; 
								?></td>
                                    <td>
									<?php 
...
if(($rowstatus['stage2'] != "0.00") and ($rowstatus['stage2'] != "")){  
		...
		if (confirm("Usted desea eliminar este pago\n- Si usted no desea eliminar este pago presione cancelar.")==true){
			window.location="payments-delete.php?id="+id;	
                           ...
                           <?php }else{ ?>  
                            <a href="payment-insurers-view.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Editar</a>
                            <?php } ?>
