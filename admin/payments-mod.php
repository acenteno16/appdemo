...
$query = "select * from payments where userid = '$_SESSION[userid]' and child = '0'".$sql." order by id desc";
$result = mysqli_query($con, $query);
$numdev = mysqli_num_rows($result);
$totpagina = ceil($numdev / $tampagina);

$query1 = "select * from payments where userid = '$_SESSION[userid]' and child = '0'".$sql." order by id desc limit ".$inicio.",".$tampagina;  
...
if($numdev > 0){  ?>
                                ...
                                <tr role="row" class="odd"><td class="sorting_1"><?php echo $row['id']; ?></td><td><?php echo $ben_name;  ?></td><td><?php 
								
								if($row['globalpayment'] != 0.00){ echo $rowcurrency['pre'].' '.$rowcurrency['symbol'].''.str_replace('.00','',number_format($row['globalpayment'], 2)); }elseif($row['payment'] != 0.00){ echo $rowcurrency['pre'].' '.$rowcurrency['symbol'].''.str_replace('.00','',number_format($row['payment'], 2)); } ?></td><td><?php 
...
if(($rowstatus['stage2'] != "0.00") and ($rowstatus['stage2'] != "")){
	...
	echo $rowstage['content'];
                            ...
                            <?php if($row['status'] == 0){ ?>
                            <a href="payment-order<? if(($row['type'] == 2) or ($row['type'] == 5)) echo "-cascade"; if($row['type'] == 3) echo "-expenses"; if($row['type'] == 4) echo "-refund"; ?>.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-edit"></i> Completar</a>
