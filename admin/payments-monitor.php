...
<?php } ?> 
    ...
    <div class="col-md-3">		
		...
		<button type="button" class="btn red" onClick="goBack();"><i class="fa fa-repeat"></i> Regresar</button>

						    <input type="hidden" id="form" name="form" value="1">
		<input type="hidden" id="monitor" name="monitor" value="1">
		<button type="submit" class="btn blue"><i class="fa fa-search"></i> Consultar</button> 
...
$query = "select payments.id from payments".$join." where payments.monitor > '0'".$sql." group by times.payment";
$result = mysqli_query($con, $query);
$numdev = mysqli_num_rows($result);
$totpagina = ceil($numdev / $tampagina);       
		
	
$query1 = "select payments.id, payments.btype, payments.provider, payments.collaborator, payments.currency, payments.payment, payments.bank, payments.status, payments.reference, payments.cnumber, payments.schedule, payments.approved, payments.today, payments.reason, payments.parent, payments.intern, payments.route, payments.globalpayment, payments.client, payments.d_approve from payments".$join." where payments.monitor > '0' and payments.id > 0".$sql." group by times.payment order by payments.id desc limit ".$inicio.",".$tampagina.""; 
