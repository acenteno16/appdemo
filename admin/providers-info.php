...
include("sessionProvidersInfo.php");  
...
$updated = isset($_GET['updated']) ? sanitizeInput($_GET['updated'], $con) :'';
		...
		<div class="page-content">
			...
			<div class="row">
				...
				<div class="col-md-12">
					...
					Información de Proveedores <?php //<small>Ordenes de pago</small> ?>
					...
					<ul class="page-breadcrumb breadcrumb">
						...
						<li>
							...
							<a href="#">Información de Proveedores</a>
...
<div class="col-md-4 ">
			    <div class="form-group">
				  <label> Código:</label>
                  <input name="code" type="text" class="form-control" id="code" value="<? echo sanitizedOutput($code); ?>">
...
<div class="col-md-4 ">
			    <div class="form-group">
				  <label> Nombre:</label>
                  <input name="name" type="text" class="form-control" id="name" value="<? echo sanitizedOutput($name); ?>"> 
...
<?
					...
					$whereClause = ""; 
					if (!empty($sql)) {
						$whereClause = implode("", $sql);
					}
					$query = "SELECT * FROM providers WHERE id > 0" . $whereClause;
					$stmt = $con->prepare($query);	
					
					if(!empty($params)){
						  $stmt->bind_param(implode("", $paramsType), ...$params);
					} 
					$stmt->execute();
...
Actualización</th>
                               ...
                               <?
									...
									$echo = isset($_POST['echo']) ? intval($_POST['echo']) : 0;
									if($echo == 1){
										echo $query; 
									}
                                ...
                                <tr role="row" class="odd <? if($row['active'] == 0) echo "danger"; ?>"><td><?php echo $row['code']; ?></td><td><?php echo $row['name']; ?></td><td><?php echo $row['ruc']; ?></td><td><?php echo $row['term']; ?> día(s)</td>
                                  <td><?php 
								   ...
								   if($row['updated'] == 1) echo "Si"; 
								    ...
								    <td><?php /*<a href="providers-view.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Ver</a>*/ ?> <a href="providers-info-view.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i>  Ver</a></td></tr><?php } ?>
