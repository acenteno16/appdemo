...
$queryproviders = "select * from workers order by first,last";
$resultproviders = mysqli_query($con, $queryproviders);
while($rowproviders = mysqli_fetch_array($resultproviders)){
...
$sql = $sql0.$sql1.$sql2.$sql3.$sql4.$sql5;
								...
								$query = "select payments.* from payments inner join workers on payments.userid = workers.code inner join bills on payments.id = bills.payment where payments.child='0' and (payments.status = '2' or payments.status = '3' or payments.status = '4') and payments.approved = '1' and ((payments.credit = 0) or (payments.credit = 2))".$sqlu.$sql." order by payments.expiration asc";  
                                ...
                                <td><?php if($rowprovider['flag'] == 1) echo '<img src="../images/flag.png" width="13" alt=""/>'; echo $ben_name; ?></td><td><?php echo $rowcurrency['pre'].' '.$rowcurrency['symbol'];
								if($row['parent'] == 0){
...
?></td><td><?php echo $rowstage['content']; ?> 
							...
							</td><td style="color:   "><a href="provision-view<? if($row['parent'] > 0) echo "-cascade"; ?>.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Ver</a> </td></tr>
                                <?php } } else { ?>
					...
					$sqlu = "";
								...
								if($numu > 0){
									...
									$query = "select payments.* from payments inner join workers on payments.userid = workers.code left join providers on payments.provider = providers.id where payments.child='0' and (payments.status = '2' or payments.status = '3' or payments.status = '4') and (providers.flag = '1' or providers.international = '1' or payments.btype = '4') and payments.approved = '1' and ((payments.credit = 0) or (payments.credit = 2))".$sqlu." order by payments.expiration asc"; 
...
?></td><td><?php echo $rowstage['content']; ?> 
							...
							</td><td><a href="provision-view<? if($row['parent'] > 0) echo "-cascade"; ?>.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Ver</a> </td></tr>
                                <?php } } else { ?>
					...
					$sqlu = "";
								...
								if($numu > 0){
									...
									$query = "select payments.* from payments inner join workers on payments.userid = workers.code where payments.child='0' and (payments.status = '2' or payments.status = '3' or payments.status = '4') and payments.approved = '1' and ((payments.credit = 0) or (payments.credit = 2))".$sqlu." order by payments.expiration asc"; 
