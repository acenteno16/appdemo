...
// Consulta preparada para obtener datos de `payments`
...
if ($row) {
    switch ($row['status']) {
        case 1:
            echo "<script>alert('Este pago no ha sido aprobado.'); window.location = 'provision-global.php'; </script>";
            break;
        case 2:
        case 3:
        case 4:
            if ($row['approved'] == 0) {
                echo "<script>alert('Este pago se encuentra en la ruta de aprobación.'); window.location = 'provision-global.php'; </script>";
            }
            break;
        case 5:
            echo "<script>alert('Este pago fue rechazado en la etapa 1.'); window.location = 'provision-global.php'; </script>";
            break;
        case 6:
            echo "<script>alert('Este pago fue rechazado en la etapa 2.'); window.location = 'provision-global.php'; </script>";
            break;
        case 7:
            echo "<script>alert('Este pago fue rechazado en la etapa 3.'); window.location = 'provision-global.php'; </script>";
            break;
        case 8:
            echo "<script>alert('Este pago ya fue provisionado.'); window.location = 'provision-global.php'; </script>";
            break;
    }
} else {
    echo "<script>alert('No se encontró el pago con el ID proporcionado.'); window.location = 'provision-global.php'; </script>";
}
...
// Consulta preparada para seleccionar datos de la tabla `bills`
...
// Inicializar las variables de los totales
$mytotalstotal = 0;
$myniototalstotal = 0;
$mybillcurrency = null;

// Procesar las filas de resultados
while ($rowaccountmaker0 = $resultaccountmaker0->fetch_assoc()) {
    // Calcular totales en `stotal` o `ammount`
    ...
    // Calcular totales en `niostotal` o `nioammount`
    ...
    // Asignar la moneda (asume que es la misma para todas las filas)
					...
					<ul class="page-breadcrumb breadcrumb">
					  <li>
						...
						<li>
							...
							<a href="provision-global.php">Provision Global</a>
				...
				<div class="col-md-12">
					...
					<div class="tabbable tabbable-custom boxless tabbable-reversed">
						...
						<div class="portlet"><div class="portlet-title">
					   	...
					   	<form action="provision-global-view-code.php" name="provisionForm" id="provisionForm" class="horizontal-form" method="post" enctype="multipart/form-data" onsubmit="return validateForm();">
                            ...
                            <? 
							$querycompany = "select companies.id, companies.name from companies inner join units on companies.code = units.companyCode where units.id = '$row[routeid]'"; 
							$resultcompany = mysqli_query($con, $querycompany);
							$rowcompany = mysqli_fetch_array($resultcompany);
							$company = $rowcompany['id'];
							?>
							
							<strong>UN:</strong> <?php echo getUnit($row['routeid'],$row['ncatalog'],'all'); ?><br>
...
font-weight: 300; font-size: 23px;">
                            ...
                            <div class="actions">
                                <? if(($row['type'] != 4) or ($_SESSION['admin'] == 'active')){ ?>
                                <a href="javascript:showTools(1);" class="btn default blue-stripe">
                                    <i class="icon-directions"></i>
                                    <span class="hidden-480">Recalcular retenciones</span>
                                </a>
                                <? } ?>
                                <? if(($row['type'] != 4) or ($_SESSION['admin'] == 'active')){ ?>
                                <a href="javascript:showTools(2);" class="btn default blue-stripe">
                                    <i class="fa fa-file"></i>
                                    <span class="hidden-480">Notas de crédito</span>
                                </a>
                                <? } 
                                if(($row['type'] == 4)/* and ($_SESSION['admin'] == 'active')*/){ ?>
                                 <a href="javascript:showTools(3);" class="btn default blue-stripe">
                                    <i class="fa fa-bank"></i>
                                    <span class="hidden-480">Comisiones bancarias</span>
                                </a>
                                <? } ?>
                            </div>
                             ...
                             <script>
								function showTools(val){
                                    if(val == 1){
                                        $("#retentionsTools").slideToggle();
                                        $("#notesTools").slideUp();
                                        $("#commissionTools").slideUp();
                                    }
                                    if(val == 2){
                                         $("#retentionsTools").slideUp();
                                         $("#notesTools").slideToggle();
                                         $("#commissionTools").slideUp();
                                    }
                                    if(val == 3){
                                         $("#retentionsTools").slideUp();
                                         $("#commissionTools").slideToggle();
                                         $("#notesTools").slideUp();
                                    }
...
<div id="retentionsTools" style="display:none;">
...
<div class="row" ><!--/span-->
          ...
          <input name="paymentadj" type="hidden" id="paymentadj" value="<?php echo $row['id']; ?>">
          <input name="provisionbillcurrency" type="hidden" id="provisionbillcurrency" value="<?php echo $rowbills['currency']; ?>">
          <input type="hidden" name="refPage" id="refPage" value="provision-global-view.php?id=<? echo $_GET['id']; ?>">
		 ...
		 $('#recalculateButton').click(function(){
			 ...
			 if((retention1 == '') || (retention2 == '')){
				 alert('Los campos de retenciones no pueden estar en blanco.');
				 return false;
			 }
...
<div id="notesTools" style="display:none;">
<div class="row">
...
<div id="commissionTools" style="display:none;">
<div class="row">
<div class="col-md-12 " ><br><br>
 ...
 <div class="col-md-12 " >
   <h3 class="form-section">Comisión bancaria</h3>
 </div>
...
<div class="col-md-2 " > 
													  <div class="form-group" >
                                                        ...
                                                        <input name="commissionamount" type="text" class="form-control" id="commissionamount" placeholder="Monto" onkeypress="return justNumbers(event);">
                                                        <br>
...
<div class="col-md-4 " > 
													  <div class="form-group" >
                                                        ...
                                                        <input name="commissioncomments" type="text" class="form-control" id="commissioncomments" placeholder="Commentarios">
                                                        <br>
...
<div class="row"></div><br><br>
          <div class="col-md-6 " >  <button type="button" class="btn blue" id="commissionButton"><i class="fa fa-check"></i> Ingresar comisión</button> 
              <input type="hidden" id="globalProvision" name="globalProvision" value="1">
          
           <script>
               $('#commissionButton').click(function(){
                   $('#provisionForm').attr('action', 'provision-commissions-add.php');
                   $('#provisionForm').attr('onsubmit', 'return validateForm4();');
                   $( "#provisionForm").submit(); 
               }); 
		  </script>

          <input name="paymentadj2" type="hidden" id="paymentadj2" value="<?php echo $_GET['id']; ?>"> <button type="button" class="btn red" id="commissionButtonDelete"><i class="fa fa-times"></i> Eliminar Comisión</button> 
              
              <script>
		 $('#commissionButtonDelete').click(function(){
             window.location = "provision-commission-delete.php?id=<?php echo $_GET['id']; ?>&global=1";
		 });
              </script>
                                             ...
                                             <div class="form-group">
                                                 ...
                                                 <?php /*<input name="linkdocument[]" type="text" class="form-control" id="linkdocument[]" placeholder="" value="">*/ ?>
                                                   <div class="input-group" id="promissoryFile">
                                                      <span class="input-group-addon" onClick="loadFiles(0);"><a href="javascript:;"><i class="fa fa-refresh"></i></a></span>
                                                      <select name="linkdocument[]" class="form-control  select2me" id="linkdocument-0" data-placeholder="Seleccionar..."> 
                                                          <option value=""></option>
                                                            <?php 
...
var noBatch = 1;
function addBatch(){
   var newBatch = '<div class="row" id="batch'+noBatch+'"><div class="col-md-3"><div class="form-group"><input name="nobatch[]" type="text" class="form-control" id="nobatch[]" placeholder="" value=""></div></div><div class="col-md-3 "><div class="form-group"><input name="nodocument[]" type="text" class="form-control" id="nodocument[]" placeholder="" value=""></div></div><div class="col-md-5 "><div class="form-group"><div class="input-group" id="promissoryFile"><span class="input-group-addon" onClick="loadFiles('+noBatch+');"><a href="javascript:;"><i class="fa fa-refresh"></i></a></span><select name="linkdocument[]" class="form-control select2me" id="linkdocument-'+noBatch+'" data-placeholder="Seleccionar..." ><option value=""></option><?php 
$queryfbox = "select * from filebox where user = '$_SESSION[userid]' order by id desc limit $global_limit";
$resultfbox = mysqli_query($con, $queryfbox);
while($rowfbox=mysqli_fetch_array($resultfbox)){
   ?><option value="<?php echo 'http://getpay.casapellas.com.ni/admin/visor.php?key='.$rowfbox['url']; ?>"><?php echo $rowfbox['id']." | ".$rowfbox['title']; ?></option><?php } ?></select></div></div></div><div class="col-md-1 "><div class="form-group"><button type="button" class="btn red" onclick="javascript:deleteRowBatch('+noBatch+');">-</button></div></div></div>';
     /*$('.select2me').select2();*/
    /*Metronic.init(); // init metronic core components*/
     noBatch++; 
...
if(($row['ret1a'] > 0) or ($row['ret2a'] > 0)){ 
	...
	$queryhalls = "select * from halls where company = '$company'"; 
...
<?php } } ?>  
       ...
       <div class="row"></div>
 <h3 class="form-section">Comentarios</h3>	
<div class="row">
	<div class="col-md-12 ">
		<div class="form-group">
		<label>Descripción:</label>
		<textarea name="pdescription" rows="2" class="form-control" id="pdescription"></textarea>
...
<label>Comentarios:</label>
  ...
  <div class="col-md-12 "><div class="form-actions right">
    ...
    <p>
			...
			window.location="provision-view-deny.php?id=<?php echo $_GET['id']; ?>&reason="+reason+"&reason2="+reason2+'&global=1'; 
		}else{
...
function loadFiles(val){
    $('#linkdocument-'+val).prop('disabled', true);
    $("#linkdocument-"+val).select2('data', { id:"0", text: "Cargando..."});
    $.post("reloadContent.php", { type: 'files', limit: <? echo $global_limit; ?> }, function(data){
        document.getElementById('linkdocument-'+val).innerHTML = data;
        $("#linkdocument-"+val).select2('data', { id:"0", text: "Seleccionar..."});
        $('#linkdocument-'+val).prop('disabled', false);
    });
}
...
function validateForm4(){
    ...
    if(amount > 0){
        if(confirm('Esta a punto de ingresar una comision bancaria. Para continuar pulse aceptar.') != true){
		  return false;
	   }
    }else{
