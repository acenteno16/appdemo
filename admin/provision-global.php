		...
		<div class="page-content">
			...
			<div class="row">
				...
				<div class="col-md-12">
					...
					Provisión Global <small>Provisión de solicitudes</small> 
...
<div class="col-md-3">
	...
	<label class="control-label">Unidades de negocio: <a href="javascript:loadUnits('load');" id="unitCharge" >[Cargar Filtro]</a></label>

						
											<select name="unit" class="form-control  select2me" id="unit" data-placeholder="Seleccionar..." disabled>
...
if(isset($_GET['provider'])){
	$filter_provider = $_GET['provider'];
                                                ...
                                                /*
$queryroutes = "select * from units where active = '1'";
...
while($rowroutes = mysqli_fetch_array($resultroutes)){
											...
											?>
                                            <option value="<?php echo $rowroutes['id']; ?>" <?php if($rowroutes["id"] == $_GET['unit']) echo 'selected'; ?>><?php echo $rowroutes['newCode'].' | '.$rowroutes["companyName"].' '.$rowroutes["lineName"].' '.$rowroutes["companyName"]; ?></option>
                                            <?php } */
											?>
                                                          ...
                                                          function loadUnits(value){
                                                              if(value == 'load'){
                                                                  $("#unit").select2('data', { id:"0", text: "Cargando Unidades..."});
                                                                  $("#unitCharge").css("display", "none");
                                                                  $.post("reloadContent.php", { type: 'routesMenu' }, function(data){
                                                                      document.getElementById("unit").innerHTML = data;
                                                                      $("#unit").select2('data', { id:"", text: "Todos las unidades"});
...
$queryproviders = "select * from companies order by name";
$resultproviders = mysqli_query($con, $queryproviders);
while($rowproviders = mysqli_fetch_array($resultproviders)){
										
											?>
                                            <option value="<?php echo $rowproviders["id"]; ?>" <?php if($rowproviders["id"] == $filter_company) echo 'selected'; ?>><?php echo $rowproviders["name"]; ?></option>
                                            <?php } ?>
...
<div class="col-md-4">							
                            ...
                            window.location = "provision-global.php";
...
<div class="portlet">
	<div class="portlet-title">
							...
							<div class="actions">
								...
								<a href="show-payments-provision-unit-export.php" class="btn default blue-stripe">
...
$sql0 = "";
if($_GET['unit'] != ""){
	$sql0 = " and payments.routeid = '$_GET[unit]'";
}
$sql1 = "";
if($_GET['provider'] != ""){
...
$sql5 = "";
if($_GET['company'] != ""){
	$sql1 = " and payments.company = '$_GET[company]'";
}	
...
$query = "select payments.id, payments.parent, payments.btype, payments.provider, payments.collaborator, payments.intern, payments.client, payments.currency, payments.fprovision, payments.payment, payments.globalpayment, payments.expiration, payments.company, payments.userid from payments".$inner." where payments.child='0' and (payments.status = '2' or payments.status = '3' or payments.status = '4') and payments.approved = '1' and ((payments.credit = 0) or (payments.credit = 2))".$sqlu.$sql." order by payments.expiration asc"; 
...
if($num > 0){ ?>
	...
	</td><td><a href="provision-global-view<? if($row['parent'] > 0) echo "-cascade"; ?>.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Ver</a></td></tr>
...
<?php }else{ ?> 
					...
					<div class="note note-success">NOTA: Debe de seleccionar un filtro para ver las solicitudes pendientes de provisión.</div>
