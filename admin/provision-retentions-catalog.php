...
<?php $query = "select * from retfamily";  
$result = mysqli_query($con, $query);
...
?>
								  ...
								  <li role="treeitem" aria-expanded="true" id="j1_1" class="jstree-node  jstree-open" aria-selected="false"><i class="jstree-icon jstree-ocl"></i><i class="jstree-icon jstree-themeicon fa fa-folder icon-state-warning icon-lg jstree-themeicon-custom"></i>
										<?php echo $row['name']; ?>
										<?php $queryscat = "select * from retfamilycontent where family = '$row[id]'";
								...
								?>
										...
										<?php echo $rowscat['code'].' | '.$rowscat['name']." (".$rowscat['percentage']."%)"; ?> <i>[Base: <? echo $rowscat['base']; ?>]</i>
                                        </li> <?php } ?></ul></li>
                                        <?php } ?>
...
function deleteCat(id){
		if (confirm("Usted desea eliminar esta categoria\n- Si usted no desea eliminar esta categoria presiona cancelar.")==true){
			window.location="categories-delete.php?id="+id;	
	} 
}
</script>
