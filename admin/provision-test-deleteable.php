		...
		<div class="page-content">
			...
			<div class="row">
				...
				<div class="col-md-12">
					...
					Provisión <small>Provisión de solicitudes</small> 
...
<div class="col-md-12">
<? if($_GET['form'] == 1){ ?>
    <button class="btn regular">Mostrar Filtro.</button>
<? }else{ ?>    
...
if($num > 0){ ?>
	...
	</td><td><a href="provision-view<? if($row['parent'] > 0) echo "-cascade"; ?>.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Ver</a> <? if(($_SESSION['email'] == 'hgaitan@casapellas.com.ni') or ($_SESSION['email'] == 'jairovargasg@gmail.com') or ($_SESSION['email'] == 'mllopez@casapellas.com.ni') and ($row['parent'] == 0)){?><a href="provision-view-new.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Ver Nuevo</a> <? }?></td></tr>  
	...
	<?php } ?>
		...
		if($_SESSION['provision_bt'] == "active"){
...
?></td><td><?php echo $rowstage['content']; ?> 
					...
					$sqlu = "";
								...
								if($numu > 0){
									...
									echo $query = "select payments.id, payments.parent, payments.btype, payments.provider, payments.collaborator, payments.intern, payments.client, payments.currency, payments.fprovision, payments.payment, payments.globalpayment, payments.expiration from payments inner join workers on payments.userid = workers.code left join providers on payments.provider = providers.id where payments.child='0' and (payments.status = '2' or payments.status = '3' or payments.status = '4') and payments.approved = '1' and ((payments.credit = 0) or (payments.credit = 2)) and providers.flag != '1' and providers.international != '1'".$sqlu." order by payments.fprovision, payments.expiration asc"; 
