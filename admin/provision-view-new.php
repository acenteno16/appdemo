			...
			<div class="row">
				...
				<div class="col-md-12">
					...
					<div class="tabbable tabbable-custom boxless tabbable-reversed">
					   	...
					   	<form action="provision-view-code-new.php" name="provisionForm" id="provisionForm" class="horizontal-form" method="post" enctype="multipart/form-data" onsubmit="return validateForm();">
									<?php 
...
<div class="col-md-3 ">												  <div class="form-group">Rets. Manuales<label>:</label>
                       ...
                       <!--/row-->
								   ...
								   <h3 class="form-section">Herramientas Pre-Provisión</h3> 
								    ...
								    <a href="javascript:showNotes();" class="icon-btn" style="height: 70px;">
							...
							<a href="javascript:showRets();" class="icon-btn" style="height: 70px;">
...
function showNotes(){
	var ndiv = document.getElementById("divNotes").style.display;
	if(ndiv == "none"){
...
function showRets(){
	var rdiv = document.getElementById("divRets").style.display;
	if(rdiv == "none"){
		document.getElementById("divRets").style.display = "block";
	}else if(rdiv == "block"){ 
		document.getElementById("divRets").style.display = "none";
	} 
}
...
<div class="col-md-2 " > 
													  <div class="form-group" >
                                                        ...
                                                        <input name="noteammount[]" type="text" class="form-control" id="noteammount[]" placeholder="Monto">
                                                        <br>
...
if (node.parentNode) {
  node.parentNode.removeChild(node);
          ...
          <div class="col-md-12">
          <button type="button" class="btn blue" id="notesButton"><i class="fa fa-check"></i> Ingresar notas de debito</button> 
          <script>
		 $('#notesButton').click(function(){
          ...
          <input name="paymentadj2" type="hidden" id="paymentadj2" value="<?php echo $_GET['id']; ?>">
          <button type="button" class="btn red" id="notesButton2"><i class="fa fa-times"></i> Eliminar notas de debito</button> 
          </div>
           <script>
...
<div id="divRets" style="display:none;">
...
<h4>Batchs, documentos y código de retenciones:</h4>								   
...
<div class="row">
<div class="col-md-2">
<span>No. Factura/Documento</span>
</div>
<div class="col-md-2"> 
<span>Monto Factura/Documento</span>	
</div>
<div class="col-md-6">
...
<div class="col-md-2"> 
	<input type="text" class="form-control" value="<? echo $curr.number_format($rowbillret['ammount'], 2); ?>" readonly>
</div>
<div class="col-md-6" id="repairThis">
	<select name="billrettype[]" id="billrettype[]" class="form-control select2me2" onChange="inactivate();">
...
<div class="col-md-3 ">
			    ...
			    <label>No. Batch:</label>
			    						<div class="input-group">
											...
											<input name="nobatch_<? echo $rowbillret['id']; ?>[]" type="text" class="form-control" id="nobatch_<? echo $rowbillret['id']; ?>[]" placeholder="" value=""></div>
...
<div class="col-md-3 ">
			    ...
			    <label>No. Documento:</label>
                                        ...
                                        <input name="nodocument_<? echo $rowbillret['id']; ?>[]" type="text" class="form-control" id="nodocument_<? echo $rowbillret['id']; ?>[]" placeholder="" value="">
...
<div class="col-md-6 " id="repairThis">
			    ...
			    <label>Link del Documento:</label>
                                                    ...
                                                    <select name="linkdocument_<? echo $rowbillret['id']; ?>[]" class="form-control  select2me2" id="linkdocument_<? echo $rowbillret['id']; ?>[]" data-placeholder="Seleccionar..." onChange="">  
...
<div id="batchwaiter_<? echo $rowbillret['id']; ?>"></div>
<div class="col-md-3 ">
<button type="button" class="btn blue" onClick="addBatch_<? echo $rowbillret['id']; ?>();">+</button>&nbsp;<button type="button" class="btn blue" onClick="openFiles();"><i class="fa fa-search"></i> Ver archivos</button>
<script>
 function openFiles(){
...
<script type="text/javascript">
var noBatch = 1;
function addBatch_<? echo $rowbillret['id']; ?>(){
   var newBatch = '<div class="" id="batch_<? echo $rowbillret['id']; ?>'+noBatch+'"><div class="col-md-3"><div class="form-group"><input name="nobatch_<? echo $rowbillret['id']; ?>[]" type="text" class="form-control" id="nobatch_<? echo $rowbillret['id']; ?>[]" placeholder="" value=""></div></div><div class="col-md-3 "><div class="form-group"><input name="nodocument_<? echo $rowbillret['id']; ?>[]" type="text" class="form-control" id="nodocument_<? echo $rowbillret['id']; ?>[]" placeholder="" value=""></div></div><div class="col-md-5 "><div class="form-group"><select name="linkdocument_<? echo $rowbillret['id']; ?>[]" class="form-control  select2me22" id="linkdocument_<? echo $rowbillret['id']; ?>[]" data-placeholder="Seleccionar..." onChange=""><option value=""></option><?php 
$queryfbox = "select * from filebox where user = '$_SESSION[userid]' order by id desc limit $global_limit"; 
$resultfbox = mysqli_query($con, $queryfbox);
while($rowfbox=mysqli_fetch_array($resultfbox)){
?><option value="<?php echo 'http://getpay.casapellas.com.ni/admin/visor.php?key='.$rowfbox['url']; ?>"><?php echo $rowfbox['id']." | ".$rowfbox['title']; ?></option><?php } ?></select></div></div><div class="col-md-1 "><div class="form-group"><button type="button" class="btn red" onclick="javascript:deleteRowBatch_<? echo $rowbillret['id']; ?>('+noBatch+');">-</button></div></div></div>';
     noBatch++; 
	 $("#batchwaiter_<? echo $rowbillret['id']; ?>").append(newBatch);
...
function deleteRowBatch_<? echo $rowbillret['id']; ?>(id){
	//document.getElementById("distribution"+id).style.display = 'none';
	var node = document.getElementById("batch_<? echo $rowbillret['id']; ?>"+id);
if (node.parentNode) {
  node.parentNode.removeChild(node);
}
}
...
<div class="row"></div>  <br>
 ...
 <div class="col-md-12"><hr style="display: block;height: 3px; border: 0; border-top: 1px solid #0A0D59; margin: 1em 0; padding: 0;"></div>
...
$querydocs = "select id from bills where payment = '$_GET[id]'";
$resultdocs = mysqli_query($con, $querydocs);
while($rowdocs=mysqli_fetch_array($resultdocs)){

?>
for (var obj in document.getElementsByName('nobatch_<? echo $rowdocs['id']; ?>[]')){
	if (i<document.getElementsByName('nobatch_<? echo $rowdocs['id']; ?>[]').length){
		varnobatch = document.getElementsByName('nobatch_<? echo $rowdocs['id']; ?>[]')[i].value;
		if(varnobatch == ''){
			alert('El campo "no batch" no puede estar en blanco');
			document.getElementsByName('nobatch_<? echo $rowdocs['id']; ?>[]')[i].focus();
			return false;
		}

		varnodocument = document.getElementsByName('nodocument_<? echo $rowdocs['id']; ?>[]')[i].value;
		if(varnodocument == ''){
			alert('El campo "no documento" no puede estar en blanco');
			document.getElementsByName('nodocument_<? echo $rowdocs['id']; ?>[]')[i].focus();
			return false;
		}

		varlinkdocument = document.getElementsByName('linkdocument_<? echo $rowdocs['id']; ?>[]')[i].value;
		if(varlinkdocument == ''){
			alert('El campo "Link del documento" no puede estar en blanco');
			document.getElementsByName('linkdocument_<? echo $rowdocs['id']; ?>[]')[i].focus();
			return false;
		}
