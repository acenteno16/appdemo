...
var payment = gstotal;
       //Retenciones en cordobas 
		...
		if(bcurrency == 1){
			...
			document.getElementById("retention1ammountnew").value = p1ammount.toFixed(2);
			var payment = parseFloat(payment)-parseFloat(p1ammount);
		}
		if(p2 != ""){
			var p2ammount = gstotalmay1000*(p2/100);
			document.getElementById("retention2ammountnew").value = p2ammount.toFixed(2);
		...
		payment = payment+" Córdobas";
		
		document.getElementById("paymentnew").value = payment;
			}
			}
		}
	...
	for(var obj in document.getElementsByName('ammount[]')){
		...
		i++
				...
				if(c2pay == 1){
					...
					paymentusd = paymentusd.toFixed(2);
					document.getElementById("floatpaymentnew").value = paymentusd;
					document.getElementById("floatcurrencynew").value = 1;
					paymentusd = "NIO C$"+paymentusd+" Córdobas"; 
				}
				else if(c2pay == 2){
		...
		//Euros 
			...
			document.getElementById("payment").value = paymentusd;
...
<div id="ddistribucion3" style="display:none;">                                        									
<div class="row">
<div class="col-md-2 ">
</div>
<div class="col-md-2 ">
...
thepercent1 = theammount*100;
thepercent = thepercent1/<?php echo $row['payment']; ?>;
...
while($rowaccountmaker=mysqli_fetch_array($resultaccountmaker)){
	...
	//Total
	//echo
	//Si el pago es en Cordobas
	if($rowaccountmaker['currency'] == 1){
	$billpaymentvar2 = str_replace('.00','',number_format($rowaccountmaker['billpayment'],2));
	}
	//si el pago es en Dolares
	if($rowaccountmaker['currency'] == 2){
	//si se paga en cordobas
	if($rowprovider['currency'] == 1){
	$billpaymentvar2 = str_replace('.00','',number_format($rowaccountmaker['niobillpayment'],2));
	}
	//si se paga en dolares
	if($rowprovider['currency'] == 2){
	$billpaymentvar2 = str_replace('.00','',number_format($rowaccountmaker['billpayment'],2));
	}
