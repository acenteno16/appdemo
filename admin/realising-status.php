...
//LIBERADA ESTE MES
$queryprovisioned = "select payments.id from payments inner join times on payments.id = times.payment where times.stage = '9' and times.today >= '$fmtoday' and payments.approved = '1' group by times.payment";
...
//Start new code
$global_inactive = 0;
$query_inactive = "select routes.* from routes where type = '6' and aa = '1'";
...
$queryprovisioned2_inactive = "select payments.* from payments inner join times on payments.id = times.payment where ((times.stage = '7.02') or (times.stage = '9') or (times.stage = '11')) and times.today >= '$fmtoday' and times.userid = '$row_inactive[worker]' group by times.payment";
...
while($row=mysqli_fetch_array($result)){
	...
	$queryprovisioned2 = "select payments.* from payments inner join times on payments.id = times.payment where (times.stage = '9') and times.today >= '$fmtoday' and times.userid = '$row[worker]' group by times.payment";
