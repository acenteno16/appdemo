...
$querylasttime = "select * from times where payment = '$id' and (stage = '8.00' or stage = '8.05') order by id desc limit 1";
...
$gcomments = "Enhorabuena, el pago ha sido Liberado.";

//time stage
$querytime = "insert into times (payment, today, now, now2, userid, stage, comment) values ('$id', '$today', '$now', '$now2', '$_SESSION[userid]', '9', '$gcomments')"; 
...
if($rowpayment['parent'] > 0){
	...
	while($rowpaymentchilds=mysqli_fetch_array($resultpaymentchilds)){
		...
		$gcommentscld = "Enhorabuena, el pago ha sido Liberado."; 

		//time stage
		$querytimecld = "insert into times (payment, today, now, now2, userid, stage, comment) values ('$rowpaymentchilds[id]', '$today', '$now', '$now2', '$_SESSION[userid]', '9', '$gcomments')"; 
