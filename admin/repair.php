...
<div class="col-md-4">
	...
	<label class="control-label">Proveedor:</label>
														  ...
														  <script>
															  $("#select2-chosen-1").click(function(){
																  alert('Hola');
															  });
														  </script>
											...
											<select name="provider" class="form-control  select2me" id="provider" data-placeholder="Seleccionar...">
												...
												<option value="">Click para cargar Proveedores</option 
 <?php 
...
$queryPayment = "select payments.id, payments.currency, payments.payment from payments inner join times on payments.id = times.payment where (times.stage = '2' or times.stage = '3' or times.stage = '4') and times.today >= '2024-01-24'"; 
...
while($rowPayment=mysqli_fetch_array($resultPayment)){
	...
	echo '<br>-'.$rowPayment['id'].' ->'.number_format($usdamount,0); 
}
...
#Repair bill TC after new units/accounting catalog

$queryPayment = "select payments.id from payments inner join times on payments.id = times.payment where times.stage = '1' and times.today >= '2024-01-05'";
$resultPayment = mysqli_query($con, $queryPayment);
while($rowPayment=mysqli_fetch_array($resultPayment)){
	
	
	$oResult = '<span style="color:green;">okay</span>';
	$queryBills = "select tc from bills where payment = '$rowPayment[id]'";
	$resultBills = mysqli_query($con, $queryBills);
	while($rowBills=mysqli_fetch_array($resultBills)){
		if($rowBills['tc'] == '0.0000'){
			$oResult = '<span style="color:red;">incomplete</span>'.$rowBills['tc'].',';
		}else{
			$oResult.=$rowBills['tc'].',';
		}
	}
