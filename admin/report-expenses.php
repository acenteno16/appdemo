					  ...
					  <li>
                        ...
                        <li>
                         ...
                         <li>
							...
							<a href="#">Tipo, concepto y categoría de gastos</a>
...
if(($_SESSION["financemanager"] == 'active') or ($_SESSION["president"] == 'active') or($_SESSION["generalmanager"] == 'active')){
	$query = "select payments.* from payments inner join times on payments.id = times.payment inner join workers on times.userid = workers.code left join bills on payments.id = bills.payment where payments.status = 14".$sql.' group by payments.id';
}
elseif($_SESSION["manager"] == 'active'){
	$query = "select payments.* from payments inner join times on payments.id = times.payment inner join workers on times.userid = workers.code left join bills on payments.id = bills.payment where payments.status = 14 and workers.unit='$_SESSION[unit]'".$sql.' group by payments.id';
}else{
...
Monto total cancelado YEN: &yen;<?php echo number_format($tyen, 2); ?>
							<div class="table-container">
								...
								<tr role="row" class="heading">
								  ...
								  <th width="3%">
									...
									<th width="16%">Tiempo de cancelación</th>
			...
			//end times
							...
							<td><?php echo date('d-m-Y',strtotime($stage[$row['id']][14])); ?></td>
							<td><?php echo $tcancellation.' días'; ?></td>
