...
$xlsRow = 2;
// 
$query = "select payments.company, payments.id, payments.btype, payments.provider, payments.collaborator, payments.intern, bills.number, bills.billdate, bills.stotal, bills.stotal2, bills.tax, bills.inturammount, bills.exempt2, bills.exempt, bills.ret1a, bills.ret2a, bills.currency, payments.cnumber, bills.tc from bills inner join payments on bills.payment = payments.id inner join times on times.payment = bills.payment where times.today >= '2018-01-16' and times.today <= '2018-01-31' and times.stage = '14'";
$result = mysqli_query($con, $query);
$num = mysqli_num_rows($result);			
while($row=mysqli_fetch_array($result)){
	
	$rowcompany = mysqli_fetch_array(mysqli_query($con, "select * from companies where id = '$row[0]'"));
	$company = $rowcompany['name']; 
	
	//Ben Type
	switch($row[2]){
		case 1:
		$rowprovider = mysqli_fetch_array(mysqli_query($con, "select * from providers where id = '$row[3]'"));
		$ben_code = $rowprovider['code'];
		$ben_name = $rowprovider['name'];
		break;
		case 2:
		$rowprovider = mysqli_fetch_array(mysqli_query($con, "select * from workers where id = '$row[4]'"));
		$ben_code = $rowprovider['code'];
		$ben_name = $rowprovider['first'].' '.$rowprovider['last'];
		break;
		case 3:
		$rowprovider = mysqli_fetch_array(mysqli_query($con, "select * from interns where code = '$row[5]'"));
		$ben_code = $rowprovider['code'];
		$ben_name = $rowprovider['first'].' '.$rowprovider['last'];
		break;
	}
	...
	//Bill info
	$billdate = date('d-m-Y', strtotime($row[7]));
	
	//Base
	//$base = $stotal-exempt2 + $stotal2-$exempt;
	$base = $row[8]+$row[9]-$row[12]-$row[13];
	$pagar = $base-$row[14]-$row[15];
	
	$rowcurrency = mysqli_fetch_array(mysqli_query($con, "select * from currency where id = '$row[16]'"));
	...
	$document_arr = array_unique($document_arr);
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				//IDS
				->setCellValue('B'.$xlsRow, $row[1])
				//Provider Code
				->setCellValue('C'.$xlsRow, $ben_code)
				//Provider Name
				->setCellValue('D'.$xlsRow, $ben_name)
				//No. de Factura 
				->setCellValue('E'.$xlsRow, $row[6])
				//Fecha de Factura
				->setCellValue('F'.$xlsRow, $billdate)
				//Monto que graba IVA
				->setCellValue('G'.$xlsRow, $row[8])
				//Monto Exento de IVA
				->setCellValue('H'.$xlsRow, $row[9])
				//IVA
				->setCellValue('I'.$xlsRow, $row[10])
				//INTUR
				->setCellValue('J'.$xlsRow, $row[11])
				//Sub Total
				->setCellValue('K'.$xlsRow, $row[8]+$row[9])
				//Exento de IMI
				->setCellValue('L'.$xlsRow, $row[12])
				//Exento de IR
				->setCellValue('M'.$xlsRow, $row[13])
				//Base para la retencion
				->setCellValue('N'.$xlsRow, $base)
				//Retenciones IR
				->setCellValue('O'.$xlsRow, $row[14])
				//Retenciones IMI
				->setCellValue('P'.$xlsRow, $row[15])
				//Monto a Pagar
				->setCellValue('Q'.$xlsRow, $pagar)
				//Batch
				->setCellValue('R'.$xlsRow, implode(', ',$batch_arr))
				//Documento
				->setCellValue('S'.$xlsRow, implode(', ',$document_arr))
				//PK
				->setCellValue('T'.$xlsRow, $row[17])
				//
				->setCellValue('U'.$xlsRow, $currencyname)
				//
				->setCellValue('V'.$xlsRow, $row[18]); 
...
header('Content-Disposition: attachment;filename="report-to-e1.xlsx"');
