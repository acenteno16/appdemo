...
if($from == ""){
	echo "<script>alert('Ingrese la fecha inicial.'); history.go(-1);</script>";
	exit(); 
}else{
	$from = date("Y-m-d", strtotime($from));
}
if($to == ""){
	echo "<script>alert('Ingrese la fecha final.'); history.go(-1);</script>";
	exit(); 
}else{
	$to = date("Y-m-d", strtotime($to));
}
if($route == ""){
	echo "<script>alert('Ingrese la Unidad de Negocio.'); history.go(-1);</script>";
	exit(); 
}
...
$query = "select payments.* from payments inner join times on payments.id = times.payment where times.stage = '13' and times.today >= '$from' and times.today <= '$to' and payments.route = '$route'";     
