		...
		<div class="page-content">
			...
			<div class="row">
				...
				<div class="col-md-12">
					...
					Reportes <small>Tiempos (Tesorería)</small> 
...
<div class="row">
<div class="col-md-6" > 
                                                    <label class="control-label">Rango de Fechas de cancelación:</label>
...
<br>
						...
						<button type="button" class="btn blue" onClick="goBack();"><i class="fa fa-repeat"></i> Regresar</button>
							...
							<script>
							function goBack(){
								window.location = "report-treasury-time.php";
							}
							</script> 
...
$query1 = "select payments.id, times.today from payments inner join times on payments.id = times.payment where times.stage = '14.00' and times.today >= '$from' and times.today <= '$to' group by times.payment limit ".$inicio.",".$tampagina;     
...
echo $num; ?> Solicitudes de pagos</div>
<div class="actions">

								<a href="<? echo $_SERVER['REQUEST_URI']; ?>&show=1" class="btn default blue-stripe">
								<i class="fa fa-desktop"></i>
								<span class="hidden-480">
								Mostrar en Pantalla</span></a>
								
								<a href="report-treasury-time-excel.php?from=<? echo $_GET['from']; ?>&to=<? echo $_GET['to']; ?>" class="btn default blue-stripe">
...
?>
								...
								<tr role="row" class="heading">
									...
									<th width="40%">
										 ...
										 Ingreso a Programacion</th>

									<th width="16%">Cancelación</th>
	...
	$days = (strtotime($today2)-strtotime($today1))/86400;								
                                ...
                                <tr role="row" class="odd"><td class="sorting_1"><?php echo $row[0]; ?></td>
                                    <td><? echo date("Y-m-d", strtotime($today1)); ?></td>
                                    <td><? echo date("Y-m-d", strtotime($today2)); ?></td>
                                    <td><? echo $days; ?></td>
                                    </tr>
                                <?php 
									$sumdays+= $days;
                                ...
                                <div>
									<p>PROM: <? echo number_format($sumdays/$i,2); ?></p>
							</div>
                 ...
                 <li>
										<a href="payments.php?page=<?php echo $previous; ?>&from=<?php echo $_GET['from']; ?>&to=<?php echo $_GET['to']; ?>&form=1">
  ...
  for ($i=1;$i<=$totpagina;$i++){
        if($pagina == $i){
		  ...
		  echo '<li><a href="report-treasury-time.php?page='.$i.'&form=1&from='.$_GET['from'].'&to='.$_GET['to'].'&show=1">'.$i."</a>"; 
		}
    } } ?>
             <?php if($next != ""){ ?>
                 ...
                 <li>
										<a href="report-treasury-time.php?page=<?php echo $next; ?>&from=<?php echo $_GET['from']; ?>&to=<?php echo $_GET['to']; ?>">
										<i class="fa fa-angle-right"></i>
										</a>
									</li>
                  <?php } ?>
                                ...
                                <?php } 
                                if(($num > 0) and ($_GET['show'] == 0)) { ?>
                                <div class="note note-success">
								<p>
								NOTA: Para ver los resultados en pantalla presione el boton de "Mostrar resultados en pantalla".
								</p>
								</div>
                                <?php } ?>
