...
foreach ($notArr as &$value) {
    $notConditions.= " and payments.status != '$value'";
}

$query = "select payments.id, payments.btype, payments.provider, payments.collaborator, payments.intern, payments.client, payments.currency, payments.userid, payments.company, payments.routeid, payments.payment, payments.solvencyExpiration from payments".$join." where payments.child='0' and payments.status >= '1' and payments.status < '14' and payments.approved != '2'".$notConditions.$sql.' group by payments.id order by payments.id desc';   
...
while($row=mysqli_fetch_array($result)){
	...
	if(!isset($thisUnitname[$row['routeid']])){
        ...
        if($row['ncatalog'] == 1){
	       ...
	       $thisUnitname[$row['routeid']] = $rowunit['code'].' | '.$rowunit['name']; 
        }
	}
