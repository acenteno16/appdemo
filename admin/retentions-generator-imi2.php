...
for($c=0;$c<sizeof($units);$c++){
	...
	$foreach_units = " and (".$foreach_units.")";
                  ...
                  <?php }
				    echo '<a href="retentions-generator-imi-global.php">- Global</a>';
				  ...
				  if($global_hall == 1){
					  $queryhall = "select * from halls where active = '1'".$foreach_company; 
				  }else{
					  $queryhall = "select * from halls where active = '1'".$foreach_company.$foreach_units; 
				  }
				  //echo $queryhall;
				  ...
				  if($numhalls == 1){
				  $rowhalls=mysqli_fetch_array($resulthall);
...
if($_GET['remission'] != ""){
	...
	$sql10 = " and hallsretention.remissionstatus = '$rstatus'";
}

$sql = $sql1.$sql2.$sql3.$sql4.$sql5.$sql6.$sql7.$sql8.$sql9.$sql10;
$ch = '&hall='.$_GET['hall'].$ch1.$ch2.$ch3.$ch4.$ch5.$ch6.$ch7.$ch8.$ch9.$ch10;                      
...
$querymain = "select hallsretention.* from hallsretention".$inner." where hallsretention.status > '0' and hallsretention.hall = '$hall'".$sql; 
...
$querymain1 = "select hallsretention.* from hallsretention".$inner." where hallsretention.status > '0' and hallsretention.hall = '$hall'".$sql." order by hallsretention.id desc limit ".$inicio.",".$tampagina;
...
if($_GET['echo'] == 1){
	echo $querymain;
	echo '<br>'.$querymain1."<br><br>"; 
}
								echo number_format($nummain,0); ?>						Retenciones <?php 							
             ...
             if(checkall.checked == true){ 
			   checkboxes[i].checked = true;
				 ...
				 checkboxes[i].checked = false;
							...
							$query = "select id, ret1a, provider, currency, routeid, hall, imiprinted from payments where id = '$rowmain[payment]'";
							...
							?>
                                ...
                                <tr role="row" class="odd <?php if($row['imiprinted'] == 1) echo 'success'; if($rowmain['void']) echo " danger"; ?>">  <td class="sorting_1" id="maintheid<?php echo $table; ?>"> 
                                  ...
                                  <td> <?php echo 'C$'.str_replace('.00','',number_format($ret1_amount,2)).' Cordobas'; ?>

</td>
                                  <td>
                                  <?php 
									echo date('d-m-Y',strtotime($rowmain['created']));
  ...
  echo date('d-m-Y',strtotime($rowretdate['today'])); 
						  ...
						  ?>
                           ...
                           <br>
                            ...
                            <a href="retentions-generator-imi-global-view-rd.php?id=<?php echo $rowmain['id']; ?>" class="btn btn-xs yellow btn-editable" target="_blank"><i class="fa fa-search"></i> RD</a><br><br>
   ...
   <div>
       ...
       ?>
								<ul class="pagination pagination-lg">
							      <?php if($pagina > 1){ ?>
                                  <li> <a href="?page=<?php echo $pagina-1; echo $ch; ?>&form=1"><i class="fa fa-angle-left"></i> </a> </li>
                                  <?php } ?>
                                    
                                    <? if($totpagina > 1){
                                     for($i=1;$i<=$totpagina;$i++){
                                        if(($i < $pagTop) and ($i > $pagLow)){ /*doNothing*/ }
                                        else{ $i = $i+9; if(($pagina + 100) < $i){ $i = $i+90; } if(($pagina - 100) > $i){ $i = $i+90; }}
                                        if($pagina == $i){ echo '<li class="active"><a href="#">' . $i . '</a></li>'; }
                                        else{ echo '<li><a href="?page='.$i.$ch.'&form=1">' . $i . '</a></li>'; }
                                     }
                                    }
    
                                    if($next != ""){ ?>
                                     <li> <a href="?page=<?php echo $pagina+1; echo $ch; ?>"><i class="fa fa-angle-right"></i> </a> </li>
                                    <?php } ?>
                            ...
                            <div class="note note-danger">
                                <p>NOTA: No se encontró ningúna retención.</p>
