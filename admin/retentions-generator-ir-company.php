...
if(!isset($_GET['company'])){
    header('location: retentions-generator-ir2.php?err=noCompany'); 
    exit();
}
...
if($_GET['company'] == ""){
    ...
    $companyid = base64_decode($_GET['company']);
    $querycompany= "select * from companies where id = '$companyid'";
    $resultcompany = mysqli_query($con, $querycompany);
    $numcompany = mysqli_num_rows($resultcompany);
    if($numcompany == 0){
        $exit = 1;
    }
}
if($exit == 1){
    exit('<script>alert("No se reconoce un ID de compañia."); window.location="retentions-generator-ir2.php";</script>');
}else{
...
<div class="col-md-12">
				...
				function removeFilter(){
					window.location = "?company=<? echo $_GET['company']; ?>";
				}
				</script>
...
$requester = $_GET['requester']; 
                             ...
                             #   '&company='.$_GET['company'].'&status=' . $_GET[ 'status' ] . '&hall=' . $_GET[ 'hall' ] . '&provider=' . $_GET[ 'provider' ] . '&from=' . $_GET[ 'from' ] . '&to=' . $_GET[ 'to' ] . '&request=' . $_GET[ 'request' ] . '&bill=' . $_GET[ 'bill' ] . '
...
$ch = '&company='.$_GET['company'].$ch1.$ch2.$ch3.$ch4.$ch5.$ch6.$ch7.$ch8.$ch9;                                
...
$querymain = "select irretention.* from irretention".$inner." where irretention.company = '$companyid'".$sql;  
...
$querymain1 = "select irretention.* from irretention".$inner." where irretention.company = '$companyid'".$sql." order by irretention.id desc limit ".$inicio.",".$tampagina;  
$resultmain1 = mysqli_query($con, $querymain1);
 

?> Retenciones IR <? echo $rowcompany['name']; ?>
...
<? } ?> 
    ...
    $pagLow = $pagina-10;
                    ...
                    <ul class="pagination pagination-lg">
                      <?php if($pagina > 1){ ?>
                      <li> <a href="?page=<?php echo $pagina-1; echo $chain; ?>&form=1"><i class="fa fa-angle-left"></i> </a> </li>
                      <?php } ?>
                      <?php
    
    
                     /*if ( $pagStart > 1 ) {
                        for ( $i = $pagStartPost; $i <= $totpagina; $i=$i+50 ) {
                          if ( $pagina == $i ) {
                      ...
                      <?php /*if($next != ""){ ?>
                      <li> <a href="payments.php?page=<?php echo $next; ?>&status=<?php echo $_GET['status']; ?>&hall=<?php echo $_GET['hall'];  ?>&provider=<?php echo $_GET['provider']; ?>&from=<?php echo $_GET['from']; ?>&to=<?php echo $_GET['to']; ?>&request=<?php echo $_GET['request']; ?>&bill=<?php echo $_GET['bill']; ?>&form=1"> <i class="fa fa-angle-right"></i> </a> </li>
                      <?php }*/ ?>
                    </ul>
                  </div>
