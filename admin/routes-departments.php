...
$query = "select * from units where managua = 0 and id > 0".$sql." group by code2 order by code";
...
?>
                                ...
                                <tr role="row" class="odd <?php if($red == 1) echo 'newred'; ?>">
                                  <td class="sorting_1"><?php if($row["newCode"] > 0) echo $row["newCode"]; else echo 'NA'; ?></td>
								  <td><?php echo $row["code2"]; ?></td>
								  <td><?php $queryname = "select * from units where code2 = '$row[code2]' order by code asc";
								  $resultname = mysqli_query($con, $queryname);
								  while($rowname=mysqli_fetch_array($resultname)){
								  echo $rowname['code']." | ".$rowname["name"].' <a href="routes-departments-special.php?unit='.$rowname['code'].'">[+]</a><br>';
								  }
						?></td>
                                  <td><a href="routes-departments-view.php?unit=<?php echo $row['code2']; ?>">
