...
?><? echo number_format($num); ?> Solicitudes en total.</div>
...
<div>
                 ...
                 <li>
										<a href="<? echo str_replace('/admin/','',$_SERVER['SCRIPT_NAME']); ?>?page=<?php echo $previous; ?>&form=1">
  ...
  for ($i=1;$i<=$totpagina;$i++){ 
        if ($pagina == $i){
		  ...
		  echo '<li><a href="'.str_replace('/admin/','',$_SERVER['SCRIPT_NAME']).'?page='.$i.'&form=1">'.$i .'</a></li>'; 
		} 
    } } ?>
             <?php if($next != ""){ ?>
                 ...
                 <li>
										<a href="<? echo str_replace('/admin/','',$_SERVER['SCRIPT_NAME']); ?>?page=<?php echo $next; ?>&form=1">
										<i class="fa fa-angle-right"></i>
										</a>
									</li>
                  <?php } ?>
