...
if($provider != ""){
    ...
    $nuevafecha = strtotime ( '+6 week' , strtotime ( $fecha ) ) ;
    $message = "Periodo maximo de 1 semana";
}
...
$query = "select payments.id, payments.description, payments.stotal, payments.company, payments.route, payments.cnumber, payments.bank, payments.btype, payments.provider, payments.collaborator, payments.intern, payments.client from payments inner join times on payments.id = times.payment where times.stage = '14'".$sql; 
$result = mysqli_query($con, $query);
#echo $num = mysqli_num_rows($result);
#exit();
function stageInfo($type, $id, $stage){
    #type: 0->date times 1->User times
    ...
    if($type == 0){
        ...
        $queryStage = "select userid from times where stage = '$stage' and payment = '$id' order by id desc limit 1";
        $resultStage = mysqli_query($con, $queryStage);
        $rowStage = mysqli_fetch_array($resultStage);
        $queryUser = "select first, last from workers where code = '$rowStage[userid]'"; 
...
function getBen($id){
    ...
    $queryPayment = "select parent, btype, provider, collaborator, intern, client from payments where id = '$id'";
    $resultPayment = mysqli_query($con, $queryPayment);
    $rowPayment = mysqli_fetch_array($resultPayment);
    
    $parent = $rowPayment['parent']; 
    $btype = $rowPayment['btype'];
    $provider = $rowPayment['provider']; 
    $collaborator = $rowPayment['collaborator'];
    $intern = $rowPayment['intern'];
    $client = $rowPayment['client'];
    
	switch($btype){
		case 1:
		$row=mysqli_fetch_array(mysqli_query($con, "select * from providers where id = '$provider'"));
		if($row['flag'] == 1){
...
/*
$ben = array();		
while($row=mysqli_fetch_array(mysqli_query($con, "select id, code, name from providers"))){
    $ben[1][$row['id']] = "$row[code] | $row[name]";     
}
while($row=mysqli_fetch_array(mysqli_query($con, "select id, code, first, last from workers"))){
    $ben[2][$row['id']] = "$row[code] | $row[first] $row[last]";   
}
while($row=mysqli_fetch_array(mysqli_query($con, "select code, first, first2, last, last2 from interns"))){
    $ben[3][$row['code']] = "$row[code] | $row[first] $row[first2] $row[last] $row[last2]";
}
while($row=mysqli_fetch_array(mysqli_query($con, "select * from clients where code = '$client'"))){
    if($row['type'] == 1){
        $ben[4][$row['code']] = "$row[code] | $row[first] $row[last]";
	}elseif($row['type'] == 2){
		$ben[4][$row['code']] = "$row[code] | $row[name]"; 
	 }
}
*/	
...
while($row=mysqli_fetch_array($result)){ 
    ...
    $requestDate = stageInfo(0, $row['id'], 1);
    $requester = stageInfo(1, $row['id'], 1);
    $cancellationDate = stageInfo(0, $row['id'], 14);
    $thisBen = getBen($row['id']);
    
    /*switch($row['btype']){
        case 1:
            $benType = $row['provider'];
            break;
        case 2:
            $benType = $row['collaborator'];
            break;
        case 3:
            $benType = $row['intern'];
            break;
