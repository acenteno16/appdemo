...
$sheet->setCellValue('D1', 'Aprobado(s) 2');
$sheet->setCellValue('E1', 'Moneda');
$sheet->setCellValue('F1', 'Monto');
$sheet->setCellValue('G1', 'Proveedor/Colaborador');
...
while($row=mysqli_fetch_array($result)){
	...
	$ltransaction = date("d-m-Y", strtotime($rowTransaction['today']));
	 ...
	 if($thisUsers[$row['routeid']] == ""){
       $users = '';
       $queryroute = "select workers.code, workers.first, workers.last from workers inner join routes on workers.code = routes.worker where routes.unitid = '$row[routeid]' and routes.type = '3'";
...
header('Content-Disposition: attachment;filename="pendientes-de-aprobado2.xlsx"');
