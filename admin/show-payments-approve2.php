...
$query = "select * from payments where status = '2' and arequest = '1' and approved = '0' and payments.child='0' order by expiration desc"; 
$result = mysqli_query($con, $query);
echo $numdev = mysqli_num_rows($result);
$totpagina = ceil($numdev / $tampagina);        


$query1 = "select * from payments where status = '2' and arequest = '1' and approved = '0' and payments.child='0' order by expiration asc limit ".$inicio.",".$tampagina;  
$result1 = mysqli_query($con, $query1); 
if($pagina < $totpagina) $next = $pagina+1;
if($pagina > 1) $previous = $pagina-1;	
								
?>
 Solicitudes en Aprobado 2</div>

							<div class="actions">

								<a href="show-payments-approve2.php<? if($_GET['detail'] == 1){ }else{ echo '?detail=1'; } ?>" class="btn default blue-stripe">
								...
								<a href="show-payments-approve2-export.php" class="btn default blue-stripe">
...
if(($rowstatus['stage2'] != "0.00") and ($rowstatus['stage2'] != "")){  
                           ...
                           <td>
                           <? 
	$queryroute = "select workers.code, workers.first, workers.last from workers inner join routes on workers.code = routes.worker where routes.unit = '$row[route]' and routes.headship = '$row[headship]' and routes.type = '5'";
	...
	while($rowroute = mysqli_fetch_array($resultroute)){
		...
		if (confirm("Usted desea eliminar este pago\n- Si usted no desea eliminar este pago presione cancelar.")==true){
			window.location="payments-delete.php?id="+id;	
                 ...
                 <li>
										<a href="show-payments-approve2.php?page=<?php echo $previous; ?>&provider=<?php echo $_GET['provider']; ?>&from=<?php echo $_GET['from']; ?>&to=<?php echo $_GET['to']; ?>&request=<?php echo $_GET['request']; ?>&bill=<?php echo $_GET['bill']; ?>&form=1">
  ...
  for ($i=1;$i<=$totpagina;$i++){ 
        if ($pagina == $i){
		  ...
		  echo '<li><a href="show-payments-approve2.php?page='.$i .'&provider='.$_GET['provider'].'&from='.$_GET['from'].'&to='.$_GET['to'].'&request='.$_GET['request'].'&bill='.$_GET['bill'].'&form=1">'.$i .'</a></li>'; 
		}
    } } ?>
             <?php if($next != ""){ ?>
                 ...
                 <li>
										<a href="show-payments-approve2.php?page=<?php echo $next; ?>&provider=<?php echo $_GET['provider']; ?>&from=<?php echo $_GET['from']; ?>&to=<?php echo $_GET['to']; ?>&request=<?php echo $_GET['request']; ?>&bill=<?php echo $_GET['bill']; ?>&form=1">
										<i class="fa fa-angle-right"></i>
										</a>
									</li>
                  <?php } ?>
