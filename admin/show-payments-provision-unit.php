...
$query1 = "select route, count(id) as numids from payments where approved = '1' and (status = '2' or status = '3' or status = '4') and child = '0' group by route order by count(id) desc"; 
...
Solicitud</th>
	...
	while($rowroute = mysqli_fetch_array($resultroute)){
		echo '<a href="mailto:'.$rowroute['email'].'">'.$rowroute['code'].' | '.$rowroute['first']." ".$rowroute['last'].'</a><br>'; 
	}
							   ?></td>
                           		<td><a href="show-payments-provision-unit-detail.php?id=<?php echo $row['route']; ?>" class="btn btn-xs default btn-editable"><i class="fa fa-search"></i> Ver</a>
								</td></tr> 
                                <?php }
