...
$query = "select payments.route, count(payments.id) as numids from payments inner join times on payments.id = times.payment where payments.approved != '2' and times.stage = '8'".$sql." group by payments.route order by count(payments.id) desc";
$result = mysqli_query($con, $query); 
