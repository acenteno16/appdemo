...
$query = "select * from payments where status = '1' and arequest = '0' and approved != '3' and approved != '2' and child='0' order by expiration desc"; 
$result = mysqli_query($con, $query);
echo $numdev = mysqli_num_rows($result);
$totpagina = ceil($numdev / $tampagina);       


$query1 = "select * from payments where status = '1' and arequest = '0' and approved != '3' and approved != '2' and child='0' order by expiration asc limit ".$inicio.",".$tampagina;  
$result1 = mysqli_query($con, $query1); 
if($pagina < $totpagina) $next = $pagina+1;
if($pagina > 1) $previous = $pagina-1;	
								
?> Solicitudes en Visto Bueno</div> 

						<div class="actions">

								
							<a href="show-payments-vobo.php<? if($_GET['detail'] == 1){ }else{ echo '?detail=1'; } ?>" class="btn default blue-stripe">
							...
							<a href="show-payments-vobo-export.php" class="btn default blue-stripe">
...
if(($rowstatus['stage2'] != "0.00") and ($rowstatus['stage2'] != "")){  
                           ...
                           <td>
                           <? 
	$queryroute = "select workers.code, workers.first, workers.last from workers inner join routes on workers.code = routes.worker where routes.unit = '$row[route]' and routes.headship = '$row[headship]' and routes.type = '20'";
...
$label = "danger";
$labelContent = 'NOTA: No hay ninguna solicitud de pago vinculada a esta cuenta.';
if($numdev > 0){ 
	$label = 'success';
