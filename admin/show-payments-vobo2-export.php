...
$query = "select payments.* from payments inner join schedulecontent on payments.id = schedulecontent.payment inner join schedule on schedulecontent.schedule = schedule.id where payments.status = '12' and payments.approved != '3' and schedule.vo = '0' order by expiration desc";    
$result = mysqli_query($con, $query);   

// Add some data
$sheet->setCellValue('A1', 'IDS');
$sheet->setCellValue('B1', 'UN');
$sheet->setCellValue('C1', 'Procesador de pago');
$sheet->setCellValue('D1', 'Moneda');
$sheet->setCellValue('E1', 'Monto');
$sheet->setCellValue('F1', 'Beneficiario');
$sheet->setCellValue('G1', 'Banco');
$sheet->setCellValue('H1', 'Compania');
...
while($row=mysqli_fetch_array($result)){
	...
	$queryroute = "select workers.code, workers.first, workers.last, workers.email from workers inner join routes on workers.code = routes.worker where routes.type = '9'";
...
header('Content-Disposition: attachment;filename="pendientes-de-vobo2.xlsx"');
