					...
					<ul class="page-breadcrumb breadcrumb">
					  <li>
						...
						<li>
							...
							<a href="#">Visto Bueno Grupo</a><i class="fa fa-angle-right"></i>
...
$query = "select payments.* from payments inner join schedulecontent on payments.id = schedulecontent.payment inner join schedule on schedulecontent.schedule = schedule.id where payments.status = '12' and payments.approved != '3' and schedule.vo = '0' order by payments.expiration desc"; 
$result = mysqli_query($con, $query);
echo $numdev = mysqli_num_rows($result);
$totpagina = ceil($numdev / $tampagina);       
//inner join schedulecontent on payments.id = schedulecontent.payment inner join schedule on schedulecontent.schedule = schedule.id
$query1 = "select payments.* from payments inner join schedulecontent on payments.id = schedulecontent.payment inner join schedule on schedulecontent.schedule = schedule.id where payments.status = '12' and payments.approved != '3' and schedule.vo = '0' order by payments.expiration desc limit ".$inicio.",".$tampagina;  
$result1 = mysqli_query($con, $query1); 
if($pagina < $totpagina) $next = $pagina+1;
if($pagina > 1) $previous = $pagina-1;	
								
?> Solicitudes en Visto Bueno Grupo</div>

						<div class="actions">

								<a href="show-payments-vobo2-export.php" class="btn default blue-stripe">
...
Solicitud</th>
                                ...
                                <tr role="row" class="odd"><td class="sorting_1"><?php echo $row['id']; ?></td><td><?php if($row['btype'] == 1){ echo $rowprovider['code']; } else{ echo $rowcollaborator['code']; } ?> |                               <?php if($rowprovider['flag'] == 1) echo '<img src="../images/flag.png" width="13"  alt=""/>'; if($row['btype'] == 1){ echo $rowprovider['name']; } else { echo $rowcollaborator['first']." ".$rowcollaborator['last']; } ?>
                                  <br>
...
if(($rowstatus['stage2'] != "0.00") and ($rowstatus['stage2'] != "")){  
	...
	while($rowroute = mysqli_fetch_array($resultroute)){
		echo $rowroute['code'].' | '.$rowroute['first']." ".$rowroute['last'].'<br>'; 
	}
							   ?>
									</td>
                           <td>
		...
		if (confirm("Usted desea eliminar este pago\n- Si usted no desea eliminar este pago presione cancelar.")==true){
			window.location="payments-delete.php?id="+id;	
                 ...
                 <li>
										<a href="show-payments-provision.php?page=<?php echo $previous; ?>&provider=<?php echo $_GET['provider']; ?>&from=<?php echo $_GET['from']; ?>&to=<?php echo $_GET['to']; ?>&request=<?php echo $_GET['request']; ?>&bill=<?php echo $_GET['bill']; ?>&form=1">
  ...
  for ($i=1;$i<=$totpagina;$i++){ 
        if ($pagina == $i){
		  ...
		  echo '<li><a href="show-payments-provision.php?page='.$i .'&provider='.$_GET['provider'].'&from='.$_GET['from'].'&to='.$_GET['to'].'&request='.$_GET['request'].'&bill='.$_GET['bill'].'&form=1">'.$i .'</a></li>'; 
		}
    } } ?>
             <?php if($next != ""){ ?>
                 ...
                 <li>
										<a href="show-payments-provision.php?page=<?php echo $next; ?>&provider=<?php echo $_GET['provider']; ?>&from=<?php echo $_GET['from']; ?>&to=<?php echo $_GET['to']; ?>&request=<?php echo $_GET['request']; ?>&bill=<?php echo $_GET['bill']; ?>&form=1">
										<i class="fa fa-angle-right"></i>
										</a>
									</li>
                  <?php } ?>
