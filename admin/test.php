...
require('headers.php');
$allowedRoles = ['admin', 'consultation'];
...
<div class="col-md-3">
    <div class="form-group">
        <label class="control-label">Proveedor: <a href="#" id="providerCharge" >[Cargar Filtro]</a></label>
...
<div class="col-md-3">
	...
	<label class="control-label">Colaborador: <a href="#" id="workerCharge" >[Cargar Filtro]</a></label>
...
<div class="col-md-3">
	...
	<label class="control-label">Pasante: <a href="#" id="internCharge" >[Cargar Filtro]</a></label>
...
<div class="col-md-3">
	...
	<label class="control-label">Clientes: <a href="#" id="clientCharge" >[Cargar Filtro]</a></label>
...
<div class="col-md-3">
	...
	<label class="control-label">Solicitante: <a href="#" id="requesterCharge" >[Cargar Filtro]</a></label>
...
<div class="col-md-3">
	...
	<label class="control-label">Ruta: <a href="#" id="routeCharge">[Cargar Filtro]</a></label>
           <select name="route" class="form-control  select2me" id="route" data-placeholder="Seleccionar..." disabled>
...
<?php if(($_SESSION['admin'] == "active") or ($_SESSION['dch'] == "active") or ($_SESSION['globalsearch'] == "active")){ ?>	
<input name="immediate" type="checkbox" id="immediate" value="1" > <label class="control-label" >Inmediato</label> 
<? } ?>
<input name="pcancellation" type="checkbox" id="pcancellation" value="1" >  <label class="control-label">Pendiente de Cancelación</label> 
<? if(($_SESSION["credit"] == 'active') or ($_SESSION['admin'] == "active")){ ?> 
<input name="credit" type="checkbox" id="credit" value="1" > <label class="control-label">Crédito</label>
<? } ?>    
</div>    
...
$sqlWhere = count($sqlConditions) > 0 ? "WHERE payments.status >= '1' AND " . implode(" AND ", $sqlConditions) : "";		
...
<? 
	...
	?>
								<span><i>Ordenadas por vencimiento</i></span>
...
if($totalResults > 0){  
                                ...
                                <td><button type="button" class="btn tooltips showDocs" data-placement="right" data-original-title="<? echo $bills; ?>" data-id="<?php echo $row['id']; ?>"><i class="fa  fa-file-o"></i></button></td>
			...
			window.location="payments-delete.php?id="+id;	
                           ...
                           <?php */ }else{ ?>  
                            ...
                            <?php } ?>
                                    ...
                                    <tr class="dNone" id="dBills<? echo $row['id']; ?>">
...
<? } ?>	
	function loadProviders(value){
  	...
  	document.getElementById('providerCharge').addEventListener('click', function(e){
		e.preventDefault();
		loadProviders('load');
	});
	...
	function loadWorkers(value){
  	...
  	document.getElementById('workerCharge').addEventListener('click', function(e){
		e.preventDefault();
		loadWorkers('load');
	});
	...
	function loadInterns(value){
  	...
  	document.getElementById('internCharge').addEventListener('click', function(e){
		e.preventDefault();
		loadInterns('load');
	});
	...
	function loadClients(value){
    	if (value === 'load') {
      	$("#client").select2('data', { id:"", text: "Cargando Clientes..." });
  	...
  	document.getElementById('clientCharge').addEventListener('click', function(e){
		e.preventDefault();
		loadClients('load');
	});
		
	function loadRequesters(value){
    	if (value === 'load') {
  	...
  	document.getElementById('requesterCharge').addEventListener('click', function(e){
		e.preventDefault();
		loadRequesters('load');
	});
	...
	function loadRoutes(value){
    	if (value === 'load') {
      	$("#route").select2('data', { id:"", text: "Cargando Rutas..." });
			$("#routeCharge").css("display", "none");
			$.post("reloadContent.php", { type: 'routesMenu' }, function(data){ 
				document.getElementById("route").innerHTML = data;
				$("#route").select2('data', { id:"", text: "Todas las Rutas" });
				$("#route").prop('disabled', false);
			});
		}
  	}

  	document.getElementById('routeCharge').addEventListener('click', function(e){
		e.preventDefault();
		loadRoutes('load');
	});
	...
	document.addEventListener('DOMContentLoaded', function() {
    	document.querySelectorAll('.showDocs').forEach(btn => {
