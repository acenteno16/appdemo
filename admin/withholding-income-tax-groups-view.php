...
<div class="page-container">
			...
			<div class="row">
                  ...
                  <div class="col-md-12"><!-- Begin: life time stats -->
                    ...
                    <div class="portlet">
						...
						<?php
						$query = "select .* from payments inner join bills on payments.id = bills.payment where payments.status >= '14' and payments.ret2a > '0'".$sql.' ';
								$querygroup = "select * from ir where id = '$_GET[id]'";
								$resultgroup = mysqli_query($con, $querygroup);
								$rowgroup = mysqli_fetch_array($resultgroup);
								
								$querymain = "select ircontent.* from ircontent inner join payments on ircontent.payment = payments.id where ircontent.package = '$_GET[id]' order by payments.ret2id asc";
								$resultmain = mysqli_query($con, $querymain);
								echo $nummain = mysqli_num_rows($resultmain);
								
								 ?> Pagos en grupo de retenciones
                    ...
                    <div class="portlet-body">
                             ...
                             <form action="#" method="post" enctype="multipart/form-data" name="form2" id="form2"><div class="table-container">
								...
								if($nummain > 0){ ?> 
                               	...
                               	<?php //echo $query; ?>
                                   <p>- Retenciones anuladas se distinguen con fondo rojo.</p>
			   ...
			   checkboxes[i].checked = true;
				 ...
				 checkboxes[i].checked = false;
							...
							$row = mysqli_fetch_array($result);
                                ...
                                <tr role="row" class="odd <?php if($row['ret2void'] == 1){ echo "danger"; } ?>"> <td class="sorting_1" id="maintheid<?php echo $table; ?>"> 
                                  <input name="theid[]" type="checkbox" id="theid[]" value="<?php echo $row['id']; ?>" class="group-checkable" data-set="#datatable_orders .theid" onChange="calculateBalance(); "  <?php if($rowmain['deleted'] == 1){ echo "disabled"; }
								 ?>></td>
                                  <td><?php echo $row['ret2id']; ?>
  ...
  ?>
                                  ...
                                  <td> <?php echo 'C$'.str_replace('.00','',number_format($row['ret2a'],2)).' Cordobas'; $gret2a+=$row['ret2a']; ?>

</td>
                                
                                <td><?php 
								$querymayorstage = "select * from irstages where id = '$row[irstage]'";
                            ...
                            <?php if($rowmain['deleted'] == 0){
								 ?>
                            <a href="payment-order-view.php?id=<?php echo $row['id']; ?>" class="btn btn-xs default btn-editable" target="_blank"><i class="fa fa-search"></i> Ver</a>
                            
                            <a href="javascript:voidRetention(<?php echo $row['id']; ?>,<?php echo $row['ret2id']; ?>);" class="btn btn-xs default btn-editable"><i class="fa fa-times"></i> Anular</a>
...
<?php if(($rowgroup['deleted'] == 0)){ ?> 
<button type="button" class="btn red" onClick="javascript:deleteGroup(<?php echo $_GET['id']; ?>);"><i class="fa fa-times"></i> Eliminar este grupo de retenciones</button>
<script>
function deleteGroup(){
	if(confirm('Esta seguro de eliminar este grupo de retenciones? Las retenciones quedaran libres para volverse a procesar pero se perdera el numero del grupo.')== true){
		window.location = 'withholding-income-tax-groups-delete.php?id=<?php echo $_GET['id']; ?>';
	} 
...
<br><br>
                     <p><strong>Total Retenciones IR:</strong> C$<?php echo number_format($gret2a, 2); ?></p>                                         
						...
						<p>
							...
							NOTA:  No hay ninguna retención deIRa por generar.  
...
function voidRetention(id,id2){
	if(confirm('Esta a punto de anular la retencion No. '+id2+' para la solicitud de pago con ID '+id+'. Seguro de anular esta retencion?')){ 
		window.location = "withholding-income-tax-group-view-void.php?id="+id;
	}
}
</script>